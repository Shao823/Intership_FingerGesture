...
//
// Data format:
//   input, streamed conv weights, folded BN scale/bias, output: signed Q8.8
//   MAC accumulator: signed 48-bit Q16.16 sum
//
...
//
// Folded BN parameters are intentionally kept in the compute block:
//   w_fold_mem[32], bias_fold_mem[32]
// They are writable only while the core is idle and start is low. Runtime writes
// are ignored so one job cannot mix old and new folded BN parameters.
//
...
//
// PE post-processing uses the same PE multiplier through an explicit mux:
//   MAC   phase: mul = activation * conv_weight
//   POST0 phase: mul = conv_q8_8 * w_fold
//   POST1 phase: add bias, saturate, ReLU
//
...
module stem_conv_array_4x4_compute #(
    ...
    reg signed [DATA_W-1:0] w_fold_mem [0:STEM_OC-1];
    reg signed [DATA_W-1:0] bias_fold_mem [0:STEM_OC-1];
...
module stem_conv_systolic_pe #(
    ...
    function signed [DATA_W-1:0] sat_q8_8_from_post;
        input signed [31:0] value;
        begin
            if (value > $signed({1'b0, {(DATA_W-1){1'b1}}})) begin
                sat_q8_8_from_post = {1'b0, {(DATA_W-1){1'b1}}};
            end else if (value < $signed({1'b1, {(DATA_W-1){1'b0}}})) begin
                sat_q8_8_from_post = {1'b1, {(DATA_W-1){1'b0}}};
            end else begin
                sat_q8_8_from_post = value[DATA_W-1:0];
            end
        end
    endfunction
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (post1_en) begin
                ...
                post_biased = post_shifted
                    + $signed({{(32-DATA_W){bn_bias[DATA_W-1]}}, bn_bias});
                bn_q = sat_q8_8_from_post(post_biased);
                post_out <= relu_q8_8(bn_q);
            end
        end
    end
