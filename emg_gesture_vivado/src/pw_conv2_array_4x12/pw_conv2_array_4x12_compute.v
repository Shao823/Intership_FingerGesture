        ...
        for (pr = 0; pr < ROWS; pr = pr + 1) begin : GEN_PE_ROW
            for (pc = 0; pc < OC_LANES; pc = pc + 1) begin : GEN_PE_COL
                ...
                pw_conv2_systolic_pe #(
                    ...
                    .a_in(a_bus[pr][pc]),
                    .w_in(w_bus[pr][pc]),
                    .bn_scale(w_fold_mem[pe_oc_index[pr][pc]]),
                    .bn_bias(bias_fold_mem[pe_oc_index[pr][pc]]),
                    .a_out(a_bus[pr][pc+1]),
                    .w_out(w_bus[pr+1][pc]),
...
module pw_conv2_systolic_pe #(
    ...
    reg signed [ACC_W-1:0] final_acc;
    reg signed [DATA_W-1:0] final_bn_scale;
    reg signed [DATA_W-1:0] final_bn_bias;
    reg final_valid;
    reg signed [DATA_W-1:0] conv_q_d;
    reg signed [DATA_W-1:0] conv_bn_scale_d;
    reg signed [DATA_W-1:0] conv_bn_bias_d;
    reg conv_valid;
    reg signed [31:0] bn_mul;
    reg signed [DATA_W-1:0] bn_bias_d;
    reg bn_valid;

    (* use_dsp = "yes" *) wire signed [31:0] mac_mul_p;
    wire signed [ACC_W-1:0] mac_product_ext;
    wire signed [ACC_W-1:0] acc_next;
    wire signed [DATA_W-1:0] final_conv_q;
    (* use_dsp = "yes" *) wire signed [31:0] bn_mul_p;
    ...
    assign final_conv_q = sat_q8_8_from_acc_shift(final_acc);
    assign bn_mul_p = $signed(conv_q_d) * $signed(conv_bn_scale_d);

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            acc <= {ACC_W{1'b0}};
            final_acc <= {ACC_W{1'b0}};
            final_bn_scale <= {DATA_W{1'b0}};
            final_bn_bias <= {DATA_W{1'b0}};
            final_valid <= 1'b0;
            conv_q_d <= {DATA_W{1'b0}};
            conv_bn_scale_d <= {DATA_W{1'b0}};
            conv_bn_bias_d <= {DATA_W{1'b0}};
            conv_valid <= 1'b0;
            bn_mul <= 32'sd0;
            bn_bias_d <= {DATA_W{1'b0}};
            bn_valid <= 1'b0;
            post_shifted <= 32'sd0;
            post_biased <= 32'sd0;
            bn_q <= {DATA_W{1'b0}};
            ...
            conv_q_d <= final_conv_q;
            conv_bn_scale_d <= final_bn_scale;
            conv_bn_bias_d <= final_bn_bias;
            conv_valid <= final_valid;

            bn_mul <= bn_mul_p;
            bn_bias_d <= conv_bn_bias_d;
            bn_valid <= conv_valid;

            if (bn_valid) begin
                post_shifted = bn_mul >>> FRAC_BITS;
                post_biased = post_shifted
                    + $signed({{(32-DATA_W){bn_bias_d[DATA_W-1]}}, bn_bias_d});
            ...
            if (mac_en) begin
                ...
                if (mac_valid) begin
                    ...
                    if (mac_last) begin
                        final_acc <= acc_next;
                        final_bn_scale <= bn_scale;
                        final_bn_bias <= bn_bias;
                        final_valid <= 1'b1;
                    end
                end
            end
