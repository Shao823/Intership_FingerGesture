...
module stem_to_dw1_buffer #(
    ...
    reg wr_meta_valid;
    reg [BANKS-1:0] wr_meta_bank_oh;
    reg [RING_ROWS-1:0] wr_meta_row_oh;
    reg [TIME_W-1:0] wr_meta_tag;
    ...
    function [RING_ROWS-1:0] ring_row_onehot;
        input [TIME_W-1:0] time_i;
        integer row_i;
        begin
            row_i = time_i % RING_ROWS;
            ring_row_onehot = {RING_ROWS{1'b0}};
            ring_row_onehot[row_i[ROW_IDX_W-1:0]] = 1'b1;
        end
    endfunction
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n || clear) begin
            ...
            wr_tile <= {TILE_W{1'b0}};
            wr_meta_valid <= 1'b0;
            wr_meta_bank_oh <= {BANKS{1'b0}};
            wr_meta_row_oh <= {RING_ROWS{1'b0}};
            wr_meta_tag <= {TIME_W{1'b0}};
        end else begin
            wr_meta_valid <= 1'b0;
            case (wr_state)
                WR_IDLE: begin
                ...
                WR_WRITE: begin
                    wr_meta_valid <= 1'b1;
                    wr_meta_bank_oh <= bank_onehot(wr_bank);
                    wr_meta_row_oh <= ring_row_onehot(wr_t_base + wr_row);
                    wr_meta_tag <= wr_t_base + wr_row;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n || clear) begin
            ...
            if (wr_meta_valid) begin
                for (rst_bank = 0; rst_bank < BANKS; rst_bank = rst_bank + 1) begin
                    for (rst_row = 0; rst_row < RING_ROWS; rst_row = rst_row + 1) begin
                        if (wr_meta_bank_oh[rst_bank]
                            && wr_meta_row_oh[rst_row]) begin
                            slot_valid[rst_bank][rst_row] <= 1'b1;
                            slot_tag[rst_bank][rst_row] <= wr_meta_tag;
                        end
                    end
                end
            end
