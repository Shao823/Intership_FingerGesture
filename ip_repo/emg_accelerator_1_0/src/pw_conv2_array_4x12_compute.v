...
module pw_conv2_array_4x12_compute #(
    ...
    reg act_cache_valid;
    (* ram_style = "registers" *) reg signed [ROWS*DATA_W-1:0] act_cache [0:PW2_IC-1];
    ...
    integer gap_acc_idx;
    integer act_cache_i;
    reg signed [ACC_W-1:0] gap_post_value;
            ...
            for (i = 0; i < ROWS; i = i + 1) begin
                for (j = 0; j < OC_LANES; j = j + 1) begin
                    ...
                    if (pe_post_valid[i][j] && row_valid_mask_reg[i]) begin
                        gap_acc_idx = (post_group_reg[i][j] * OC_LANES) + j;
                        gap_post_value = $signed({
                            {(ACC_W-DATA_W){post_bus[i][j][DATA_W-1]}},
                            post_bus[i][j]
                        });
                        gap_sum_valid[gap_acc_idx] <= 1'b1;
                        if (gap_sum_valid[gap_acc_idx]) begin
                            gap_sum_mem[gap_acc_idx]
                                <= gap_sum_mem[gap_acc_idx] + gap_post_value;
                        end else begin
                            gap_sum_mem[gap_acc_idx] <= gap_post_value;
                        end
                    end
