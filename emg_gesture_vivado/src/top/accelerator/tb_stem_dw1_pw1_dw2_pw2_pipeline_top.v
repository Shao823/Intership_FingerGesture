...
module tb_stem_dw1_pw1_dw2_pw2_pipeline_top #(
    ...
    parameter integer RUN_CASE_LIMIT = 10,
    ...
    parameter integer MAX_MISMATCH_PRINTS = 20,
    parameter integer CHECK_QUANT_PARAMS = 1
);
    ...
    localparam integer TOTAL_PW2_POST = POOL_LEN * PW2_OC;
    ...
    reg signed [MULT_W-1:0] pw2_w_fold_wdata;
    reg pw2_bias_fold_we;
    reg [6:0] pw2_bias_fold_oc;
    reg signed [31:0] pw2_bias_fold_wdata;
    ...
    integer pw2_post_count;
    integer stem_error_count;
    integer dw1_error_count;
    integer pw1_error_count;
    integer dw2_error_count;
    integer pw2_error_count;
    integer gap_error_count;
    integer fc_error_count;
    integer count_error_count;
    integer param_error_count;
    ...
    reg signed [DATA_W-1:0] file_expected_pw2 [0:N_CASES*POOL_LEN*PW2_OC-1];
    ...
    reg seen_pw2 [0:TOTAL_PW2_POST-1];

    reg signed [31:0] exp_stem_bias [0:STEM_OC-1];
    reg signed [MULT_W-1:0] exp_stem_mult [0:STEM_OC-1];
    reg [15:0] exp_stem_shift [0:STEM_OC-1];
    reg signed [31:0] exp_dw1_bias [0:STEM_OC-1];
    reg signed [MULT_W-1:0] exp_dw1_mult [0:STEM_OC-1];
    reg [15:0] exp_dw1_shift [0:STEM_OC-1];
    reg signed [31:0] exp_pw1_bias [0:PW1_OC-1];
    reg signed [MULT_W-1:0] exp_pw1_mult [0:PW1_OC-1];
    reg [15:0] exp_pw1_shift [0:PW1_OC-1];
    reg signed [31:0] exp_dw2_bias [0:PW1_OC-1];
    reg signed [MULT_W-1:0] exp_dw2_mult [0:PW1_OC-1];
    reg [15:0] exp_dw2_shift [0:PW1_OC-1];
    reg signed [31:0] exp_pw2_bias [0:PW2_OC-1];
    reg signed [MULT_W-1:0] exp_pw2_mult [0:PW2_OC-1];
    reg [15:0] exp_pw2_shift [0:PW2_OC-1];
    reg signed [MULT_W-1:0] exp_gap_mult [0:0];
    reg [15:0] exp_gap_shift [0:0];
    reg signed [31:0] exp_fc0_bias [0:PW2_OC-1];
    reg signed [MULT_W-1:0] exp_fc0_mult [0:PW2_OC-1];
    reg [15:0] exp_fc0_shift [0:PW2_OC-1];
    reg signed [31:0] exp_fc1_bias [0:FC_CLASSES-1];
    reg signed [MULT_W-1:0] exp_fc1_mult [0:FC_CLASSES-1];
    reg [15:0] exp_fc1_shift [0:FC_CLASSES-1];
    ...
    initial begin
        $readmemh({`PROJECT_ROOT, "/emg_gesture_vivado/src/top/accelerator/testdata/raw_input.mem"}, file_raw_input);
        $readmemh({`PROJECT_ROOT, "/emg_gesture_vivado/src/top/accelerator/testdata/stem_expected_full.mem"}, file_expected_stem);
        $readmemh({`PROJECT_ROOT, "/emg_gesture_vivado/src/top/accelerator/testdata/dw1_expected_full.mem"}, file_expected_dw1);
        $readmemh({`PROJECT_ROOT, "/emg_gesture_vivado/src/top/accelerator/testdata/pw1_expected_pool.mem"}, file_expected_pw1);
        $readmemh({`PROJECT_ROOT, "/emg_gesture_vivado/src/top/accelerator/testdata/dw2_expected_full.mem"}, file_expected_dw2);
        $readmemh({`PROJECT_ROOT, "/emg_gesture_vivado/src/top/accelerator/testdata/pw2_expected_post.mem"}, file_expected_pw2);
        $readmemh({`PROJECT_ROOT, "/emg_gesture_vivado/src/top/accelerator/testdata/pw2_gap_sum.mem"}, file_expected_gap);
        $readmemh({`PROJECT_ROOT, "/emg_gesture_vivado/src/top/accelerator/testdata/fc_expected_logits.mem"}, file_expected_fc_logits);
        $readmemh({`PROJECT_ROOT, "/emg_gesture_vivado/src/top/accelerator/testdata/fc_expected_class.mem"}, file_expected_fc_class);

        $readmemh({`PROJECT_ROOT, "/weight_data/stem/bias_int32.mem"}, exp_stem_bias);
        $readmemh({`PROJECT_ROOT, "/weight_data/stem/multiplier_q15.mem"}, exp_stem_mult);
        $readmemh({`PROJECT_ROOT, "/weight_data/stem/right_shift.mem"}, exp_stem_shift);
        $readmemh({`PROJECT_ROOT, "/weight_data/dw1/bias_int32.mem"}, exp_dw1_bias);
        $readmemh({`PROJECT_ROOT, "/weight_data/dw1/multiplier_q15.mem"}, exp_dw1_mult);
        $readmemh({`PROJECT_ROOT, "/weight_data/dw1/right_shift.mem"}, exp_dw1_shift);
        $readmemh({`PROJECT_ROOT, "/weight_data/pw1/bias_int32.mem"}, exp_pw1_bias);
        $readmemh({`PROJECT_ROOT, "/weight_data/pw1/multiplier_q15.mem"}, exp_pw1_mult);
        $readmemh({`PROJECT_ROOT, "/weight_data/pw1/right_shift.mem"}, exp_pw1_shift);
        $readmemh({`PROJECT_ROOT, "/weight_data/dw2/bias_int32.mem"}, exp_dw2_bias);
        $readmemh({`PROJECT_ROOT, "/weight_data/dw2/multiplier_q15.mem"}, exp_dw2_mult);
        $readmemh({`PROJECT_ROOT, "/weight_data/dw2/right_shift.mem"}, exp_dw2_shift);
        $readmemh({`PROJECT_ROOT, "/weight_data/pw2/bias_int32.mem"}, exp_pw2_bias);
        $readmemh({`PROJECT_ROOT, "/weight_data/pw2/multiplier_q15.mem"}, exp_pw2_mult);
        $readmemh({`PROJECT_ROOT, "/weight_data/pw2/right_shift.mem"}, exp_pw2_shift);
        $readmemh({`PROJECT_ROOT, "/weight_data/gap/multiplier_q15.mem"}, exp_gap_mult);
        $readmemh({`PROJECT_ROOT, "/weight_data/gap/right_shift.mem"}, exp_gap_shift);
        $readmemh({`PROJECT_ROOT, "/weight_data/fc0/bias_int32.mem"}, exp_fc0_bias);
        $readmemh({`PROJECT_ROOT, "/weight_data/fc0/multiplier_q15.mem"}, exp_fc0_mult);
        $readmemh({`PROJECT_ROOT, "/weight_data/fc0/right_shift.mem"}, exp_fc0_shift);
        $readmemh({`PROJECT_ROOT, "/weight_data/fc1/bias_int32.mem"}, exp_fc1_bias);
        $readmemh({`PROJECT_ROOT, "/weight_data/fc1/multiplier_q15.mem"}, exp_fc1_mult);
        $readmemh({`PROJECT_ROOT, "/weight_data/fc1/right_shift.mem"}, exp_fc1_shift);
    end
    ...
    task clear_scoreboard;
        begin
            ...
            for (seen_i = 0; seen_i < TOTAL_PW2_POST; seen_i = seen_i + 1) begin
                seen_pw2[seen_i] = 1'b0;
            end
        end
    endtask

    task check_param_value;
        ...
        begin
            if (got !== expected) begin
                if (param_error_count < MAX_MISMATCH_PRINTS) begin
                    $display(
                        "ERROR param %0s[%0d] got=%0d expected=%0d got_hex=%h expected_hex=%h",
                        label,
                        idx,
                        got,
                        expected,
                ...
                param_error_count = param_error_count + 1;
                errors = errors + 1;
            end
        end
    endtask

    task check_quant_params;
        begin
            param_error_count = 0;
            if (CHECK_QUANT_PARAMS) begin
                for (param_i = 0; param_i < STEM_OC; param_i = param_i + 1) begin
                    check_param_value("stem.bias", param_i,
                        {{32{dut.u_stem.u_compute.bias_int32_mem[param_i][31]}},
                            dut.u_stem.u_compute.bias_int32_mem[param_i]},
                        {{32{exp_stem_bias[param_i][31]}}, exp_stem_bias[param_i]});
                    check_param_value("stem.mult", param_i,
                        {{(64-MULT_W){dut.u_stem.u_compute.multiplier_q15_mem[param_i][MULT_W-1]}},
                            dut.u_stem.u_compute.multiplier_q15_mem[param_i]},
                        {{(64-MULT_W){exp_stem_mult[param_i][MULT_W-1]}},
                            exp_stem_mult[param_i]});
                    check_param_value("stem.shift", param_i,
                        {56'd0, dut.u_stem.u_compute.right_shift_mem[param_i]},
                        {56'd0, exp_stem_shift[param_i][7:0]});
                end

                for (param_i = 0; param_i < STEM_OC; param_i = param_i + 1) begin
                    check_param_value("dw1.bias", param_i,
                        {{32{dut.u_dw1.bias_int32_mem[param_i][31]}},
                            dut.u_dw1.bias_int32_mem[param_i]},
                        {{32{exp_dw1_bias[param_i][31]}}, exp_dw1_bias[param_i]});
                    check_param_value("dw1.mult", param_i,
                        {{(64-MULT_W){dut.u_dw1.multiplier_q15_mem[param_i][MULT_W-1]}},
                            dut.u_dw1.multiplier_q15_mem[param_i]},
                        {{(64-MULT_W){exp_dw1_mult[param_i][MULT_W-1]}},
                            exp_dw1_mult[param_i]});
                    check_param_value("dw1.shift", param_i,
                        {56'd0, dut.u_dw1.right_shift_mem[param_i]},
                        {56'd0, exp_dw1_shift[param_i][7:0]});
                end

                for (param_i = 0; param_i < PW1_OC; param_i = param_i + 1) begin
                    check_param_value("pw1.bias", param_i,
                        {{32{dut.u_pw1.u_compute.bias_int32_mem[param_i][31]}},
                            dut.u_pw1.u_compute.bias_int32_mem[param_i]},
                        {{32{exp_pw1_bias[param_i][31]}}, exp_pw1_bias[param_i]});
                    check_param_value("pw1.mult", param_i,
                        {{(64-MULT_W){dut.u_pw1.u_compute.multiplier_q15_mem[param_i][MULT_W-1]}},
                            dut.u_pw1.u_compute.multiplier_q15_mem[param_i]},
                        {{(64-MULT_W){exp_pw1_mult[param_i][MULT_W-1]}},
                            exp_pw1_mult[param_i]});
                    check_param_value("pw1.shift", param_i,
                        {56'd0, dut.u_pw1.u_compute.right_shift_mem[param_i]},
                        {56'd0, exp_pw1_shift[param_i][7:0]});
                end

                for (param_i = 0; param_i < PW1_OC; param_i = param_i + 1) begin
                    check_param_value("dw2.bias", param_i,
                        {{32{dut.u_dw2.bias_int32_mem[param_i][31]}},
                            dut.u_dw2.bias_int32_mem[param_i]},
                        {{32{exp_dw2_bias[param_i][31]}}, exp_dw2_bias[param_i]});
                    check_param_value("dw2.mult", param_i,
                        {{(64-MULT_W){dut.u_dw2.multiplier_q15_mem[param_i][MULT_W-1]}},
                            dut.u_dw2.multiplier_q15_mem[param_i]},
                        {{(64-MULT_W){exp_dw2_mult[param_i][MULT_W-1]}},
                            exp_dw2_mult[param_i]});
                    check_param_value("dw2.shift", param_i,
                        {56'd0, dut.u_dw2.right_shift_mem[param_i]},
                        {56'd0, exp_dw2_shift[param_i][7:0]});
                end

                for (param_i = 0; param_i < PW2_OC; param_i = param_i + 1) begin
                    check_param_value("pw2.bias", param_i,
                        {{32{dut.u_pw2.u_compute.bias_int32_mem[param_i][31]}},
                            dut.u_pw2.u_compute.bias_int32_mem[param_i]},
                        {{32{exp_pw2_bias[param_i][31]}}, exp_pw2_bias[param_i]});
                    check_param_value("pw2.mult", param_i,
                        {{(64-MULT_W){dut.u_pw2.u_compute.multiplier_q15_mem[param_i][MULT_W-1]}},
                            dut.u_pw2.u_compute.multiplier_q15_mem[param_i]},
                        {{(64-MULT_W){exp_pw2_mult[param_i][MULT_W-1]}},
                            exp_pw2_mult[param_i]});
                    check_param_value("pw2.shift", param_i,
                        {56'd0, dut.u_pw2.u_compute.right_shift_mem[param_i]},
                        {56'd0, exp_pw2_shift[param_i][7:0]});
                end

                check_param_value("gap.mult", 0,
                    {{(64-MULT_W){dut.u_fc.u_compute.gap_multiplier_q15_mem[0][MULT_W-1]}},
                        dut.u_fc.u_compute.gap_multiplier_q15_mem[0]},
                    {{(64-MULT_W){exp_gap_mult[0][MULT_W-1]}}, exp_gap_mult[0]});
                check_param_value("gap.shift", 0,
                    {56'd0, dut.u_fc.u_compute.gap_right_shift_mem[0]},
                    {56'd0, exp_gap_shift[0][7:0]});

                for (param_i = 0; param_i < PW2_OC; param_i = param_i + 1) begin
                    check_param_value("fc0.bias", param_i,
                        {{32{dut.u_fc.u_compute.fc0_bias_int32_mem[param_i][31]}},
                            dut.u_fc.u_compute.fc0_bias_int32_mem[param_i]},
                        {{32{exp_fc0_bias[param_i][31]}}, exp_fc0_bias[param_i]});
                    check_param_value("fc0.mult", param_i,
                        {{(64-MULT_W){dut.u_fc.u_compute.fc0_multiplier_q15_mem[param_i][MULT_W-1]}},
                            dut.u_fc.u_compute.fc0_multiplier_q15_mem[param_i]},
                        {{(64-MULT_W){exp_fc0_mult[param_i][MULT_W-1]}},
                            exp_fc0_mult[param_i]});
                    check_param_value("fc0.shift", param_i,
                        {56'd0, dut.u_fc.u_compute.fc0_right_shift_mem[param_i]},
                        {56'd0, exp_fc0_shift[param_i][7:0]});
                end

                for (param_i = 0; param_i < FC_CLASSES; param_i = param_i + 1) begin
                    check_param_value("fc1.bias", param_i,
                        {{32{dut.u_fc.u_compute.fc1_bias_int32_mem[param_i][31]}},
                            dut.u_fc.u_compute.fc1_bias_int32_mem[param_i]},
                        {{32{exp_fc1_bias[param_i][31]}}, exp_fc1_bias[param_i]});
                    check_param_value("fc1.mult", param_i,
                        {{(64-MULT_W){dut.u_fc.u_compute.fc1_multiplier_q15_mem[param_i][MULT_W-1]}},
                            dut.u_fc.u_compute.fc1_multiplier_q15_mem[param_i]},
                        {{(64-MULT_W){exp_fc1_mult[param_i][MULT_W-1]}},
                            exp_fc1_mult[param_i]});
                    check_param_value("fc1.shift", param_i,
                        {56'd0, dut.u_fc.u_compute.fc1_right_shift_mem[param_i]},
                        {56'd0, exp_fc1_shift[param_i][7:0]});
                end

                if (param_error_count == 0) begin
                    $display("INFO quant params PASS stem/dw1/pw1/dw2/pw2/gap/fc0/fc1");
                end else begin
                    $display("ERROR quant params mismatches=%0d", param_error_count);
                end
            end
        end
    endtask
            ...
            for (row = 0; row < ROWS; row = row + 1) begin
                ...
                for (lane = 0; lane < STEM_OC_LANES; lane = lane + 1) begin
                    ...
                    if (get_stem_lane(row, lane) !== expected) begin
                        if (stem_error_count < MAX_MISMATCH_PRINTS) begin
                        ...
                        stem_error_count = stem_error_count + 1;
                        errors = errors + 1;
                    end
                end
            end
        ...
        begin
            ...
            for (lane_i = 0; lane_i < STEM_OC_LANES; lane_i = lane_i + 1) begin
                ...
                if (get_dw1_lane(lane_i) !== expected) begin
                    if (dw1_error_count < MAX_MISMATCH_PRINTS) begin
                    ...
                    dw1_error_count = dw1_error_count + 1;
                    errors = errors + 1;
                end
            end
            ...
            for (pool_row = 0; pool_row < POOL_ROWS; pool_row = pool_row + 1) begin
                ...
                for (lane = 0; lane < PW1_OC_LANES; lane = lane + 1) begin
                    ...
                    if (get_pw1_lane(pool_row, lane) !== expected) begin
                        if (pw1_error_count < MAX_MISMATCH_PRINTS) begin
                        ...
                        pw1_error_count = pw1_error_count + 1;
                        errors = errors + 1;
                    end
                end
            end
        ...
        begin
            ...
            for (lane_i = 0; lane_i < CH_LANES; lane_i = lane_i + 1) begin
                ...
                if (get_dw2_lane(lane_i) !== expected) begin
                    if (dw2_error_count < MAX_MISMATCH_PRINTS) begin
                    ...
                    dw2_error_count = dw2_error_count + 1;
                    errors = errors + 1;
                end
            end
    ...
    task check_pw2_post_outputs;
        ...
        begin
            for (row = 0; row < ROWS; row = row + 1) begin
                for (col = 0; col < PW2_OC_LANES; col = col + 1) begin
                    if (dut.u_pw2.u_compute.pe_post_valid[row][col]
                        && dut.u_pw2.u_compute.row_valid_mask_reg[row]) begin
                        group = dut.u_pw2.u_compute.post_group_reg[row][col];
                        t_idx = dut.u_pw2.u_compute.tile_t_base_reg + row;
                        oc_idx = group * PW2_OC_LANES + col;

                        if ((t_idx >= POOL_LEN) || (oc_idx >= PW2_OC)) begin
                            if (count_error_count < MAX_MISMATCH_PRINTS) begin
                                $display(
                                    "ERROR PW2 post index out of range case=%0d t=%0d oc=%0d group=%0d row=%0d col=%0d",
                                    current_case, t_idx, oc_idx, group, row, col
                                );
                            end
                            count_error_count = count_error_count + 1;
                            errors = errors + 1;
                        end else begin
                            seen_idx = t_idx * PW2_OC + oc_idx;
                            if (seen_pw2[seen_idx]) begin
                                if (count_error_count < MAX_MISMATCH_PRINTS) begin
                                    $display(
                                        "ERROR duplicate PW2 post case=%0d t=%0d oc=%0d",
                                        current_case, t_idx, oc_idx
                                    );
                                end
                                count_error_count = count_error_count + 1;
                                errors = errors + 1;
                            end
                            seen_pw2[seen_idx] = 1'b1;

                            expected_idx = current_case*POOL_LEN*PW2_OC
                                + t_idx*PW2_OC + oc_idx;
                            expected = file_expected_pw2[expected_idx];
                            got = dut.u_pw2.u_compute.post_bus[row][col];
                            if (got !== expected) begin
                                if (pw2_error_count < MAX_MISMATCH_PRINTS) begin
                                    $display(
                                        "ERROR PW2 case=%0d t=%0d oc=%0d group=%0d row=%0d col=%0d got=%0d expected=%0d",
                                        current_case,
                                        t_idx,
                                        oc_idx,
                                        group,
                                ...
                                pw2_error_count = pw2_error_count + 1;
                                errors = errors + 1;
                            end
                            pw2_post_count = pw2_post_count + 1;
                        end
                    end
                end
            end
    ...
    task check_gap;
        ...
        begin
            if (gap_count !== POOL_LEN[8:0]) begin
                if (gap_error_count < MAX_MISMATCH_PRINTS) begin
                    $display("ERROR gap_count got=%0d expected=%0d", gap_count, POOL_LEN);
                end
                gap_error_count = gap_error_count + 1;
                errors = errors + 1;
            end
    ...
    task check_final_counts;
        begin
            ...
            if (pw2_post_count != TOTAL_PW2_POST) begin
                $display("ERROR PW2 post_count got=%0d expected=%0d", pw2_post_count, TOTAL_PW2_POST);
                count_error_count = count_error_count + 1;
                errors = errors + 1;
            end
            ...
            for (out_idx = 0; out_idx < TOTAL_PW2_POST; out_idx = out_idx + 1) begin
                if (!seen_pw2[out_idx]) begin
                    if (count_error_count < MAX_MISMATCH_PRINTS) begin
                        $display("ERROR missing PW2 post output t=%0d oc=%0d",
                            out_idx / PW2_OC, out_idx % PW2_OC);
                    end
                    count_error_count = count_error_count + 1;
                    errors = errors + 1;
                end
    ...
    task run_preloaded_case_with_optional_next_load;
        ...
        begin
            ...
            if (done !== 1'b1) begin
                ...
                check_fc_result();
                $display(
                    "CASE %0d cycles=%0d gap_count=%0d class=%0d stem_tiles=%0d dw1_rows=%0d pw1_out=%0d dw2_rows=%0d pw2_post=%0d pw2_tiles=%0d fc=%0d/%0d max_dw1_pending=%0d max_dw2_pending=%0d waits p1=%0d p2=%0d stalls d1=%0d d2=%0d overwrite d1p1=%0d p1d2=%0d d2p2=%0d errors_delta=%0d",
                    case_idx,
                    cycle_count,
                    gap_count,
                    fc_class_idx,
                ...
                $display(
                    "CASE %0d stage_errors stem=%0d dw1=%0d pw1=%0d dw2=%0d pw2=%0d gap=%0d fc=%0d count=%0d",
                    case_idx,
                    stem_error_count,
                    dw1_error_count,
                    pw1_error_count,
