...
module tb_pw_conv2_weight_controller_with_rom;
    ...
    localparam integer ROM_DEPTH  = PW2_OC / OC_LANES * PW2_IC;
    localparam integer ADDR_W     = (ROM_DEPTH <= 1) ? 1 : $clog2(ROM_DEPTH);
    localparam integer MAX_REQS   = 1024;
    localparam integer TIMEOUT    = 3500;
    // This TB instantiates the generated blk_mem_gen_pw_conv2_weight IP, whose
    // current .xci has READ_LATENCY_A=1.
    ...
    initial begin
        clk = 1'b0;
        $readmemh({`PROJECT_ROOT, "/weight_data/pw2/weight_packed.mem"}, expected_mem);
    end
    ...
    function integer calc_addr;
        ...
        input [6:0] k_i;
        begin
            calc_addr = (oc_base_i / OC_LANES) * PW2_IC + k_i;
        end
    endfunction

    function [6:0] req_oc_for_index;
        input integer idx;
        integer group;
        begin
            group = (idx / PW2_IC) % (PW2_OC / OC_LANES);
            req_oc_for_index = group * OC_LANES;
        end
    endfunction

    function [6:0] req_k_for_index;
        input integer idx;
        begin
            req_k_for_index = idx % PW2_IC;
        end
    endfunction
    ...
    task check_expected_mem_loaded;
        begin
            if ((^expected_mem[0]) === 1'bx) begin
                $display("ERROR PW2 expected weight_packed.mem did not load");
                errors = errors + 1;
            end
        end
    endtask

    task run_autoload_probe;
        integer probe_timeout;
        integer accepted;
        integer responded;
        begin
            check_expected_mem_loaded();
            clear_scoreboard();
            probe_timeout = 0;
            accepted = 0;
            responded = 0;
            ...
            while (!accepted && (probe_timeout < 20)) begin
                #1;
            ...
            while (!responded && (probe_timeout < 80)) begin
                #1;
                if (weight_vec_valid && weight_vec_ready) begin
                    responded = 1;
                    if (weight_vec !== expected_mem[0]) begin
                        $display(
                            "ERROR PW2 ROM autoload probe mismatch got=%h expected=%h",
                            weight_vec,
                            expected_mem[0]
                        );
                        errors = errors + 1;
            ...
            if (!accepted) begin
                $display("ERROR PW2 ROM autoload probe request was not accepted");
                errors = errors + 1;
            end
            if (!responded) begin
                $display("ERROR PW2 ROM autoload probe did not receive a response");
                errors = errors + 1;
            end

            @(negedge clk);
            weight_vec_ready = 1'b1;
            $display("INFO PW2 IP autoload_probe complete");
        end
    endtask
        ...
        begin
            ...
            if (addr < 0 || addr >= ROM_DEPTH) begin
                ...
                if (dut.rom_en !== 1'b1) begin
                    $display("ERROR PW2 IP rom_en not asserted for accepted request");
                    errors = errors + 1;
                end
                if (dut.rom_addr !== addr[ADDR_W-1:0]) begin
                    $display("ERROR PW2 IP rom_addr got=%0d expected=%0d oc_base=%0d k=%0d",
                ...
                if (seen_addr[addr]) begin
                    $display("ERROR PW2 IP duplicate ROM address in one sequence addr=%0d", addr);
                    errors = errors + 1;
                end else begin
                    seen_addr[addr] = 1'b1;
    ...
    task run_sequence;
        ...
        begin
            ...
            $display(
                "INFO PW2 IP L%0d %0s requests=%0d responses=%0d cycles=%0d req_stalls=%0d req_gaps=%0d first_gap=%0d unique_addrs=%0d",
                BRAM_READ_LATENCY,
                label,
                total_requests,
                total_responses,
    ...
    task run_invalid_request_case;
        ...
        begin
            ...
            send_invalid_request(7'd5, 7'd0);
            send_invalid_request(7'd0, 7'd64);
            invalid_after = dut.u_ctrl.invalid_req_count;
            if ((invalid_after - invalid_before) != 2) begin
                $display(
                    "ERROR PW2 IP invalid_req_count delta got=%0d expected=2",
                    invalid_after - invalid_before
                );
                errors = errors + 1;
            end
            $display("INFO PW2 IP invalid_request_case invalid_delta=%0d",
                invalid_after - invalid_before);
        end
    endtask
    ...
    initial begin
        ...
        reset_dut();
        run_sequence(90, READY_ALWAYS, REQ_CONTINUOUS, 0, 0, "pipeline_ready_always");
        ...
        run_sequence(
            ...
            1,
            0,
            "all_512_words_periodic_backpressure"
        );

        reset_dut();
        run_sequence(24, READY_HEAD_LOW, REQ_CONTINUOUS, 1, 0, "head_backpressure");

        reset_dut();
        run_sequence(48, READY_ALWAYS, REQ_MID_GAP, 0, 1, "request_valid_mid_gap");

        reset_dut();
        run_invalid_request_case();

        if (errors == 0) begin
            $display("PASS tb_pw_conv2_weight_controller_with_rom");
        end else begin
            $display("FAIL tb_pw_conv2_weight_controller_with_rom errors=%0d", errors);
            $finish;
        end
        $finish;
    end
