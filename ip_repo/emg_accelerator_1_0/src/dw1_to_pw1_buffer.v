...
module dw1_to_pw1_buffer #(
    ...
    initial begin
        ...
        if (ROWS != 4) begin
            $error("dw1_to_pw1_buffer: this implementation requires ROWS == 4");
        end
    ...
    function signed [ACT_W-1:0] make_act_vec_from_cache;
        ...
        begin
            ...
            word_i = tile_cache_row0[bank_i];
            make_act_vec_from_cache[0*DATA_W +: DATA_W]
                = word_i[lane_i*DATA_W +: DATA_W];

            word_i = tile_cache_row1[bank_i];
            make_act_vec_from_cache[1*DATA_W +: DATA_W]
                = word_i[lane_i*DATA_W +: DATA_W];

            word_i = tile_cache_row2[bank_i];
            make_act_vec_from_cache[2*DATA_W +: DATA_W]
                = word_i[lane_i*DATA_W +: DATA_W];

            word_i = tile_cache_row3[bank_i];
            make_act_vec_from_cache[3*DATA_W +: DATA_W]
                = word_i[lane_i*DATA_W +: DATA_W];
        end
    endfunction

    function [BANK_W-1:0] load_cache_bank;
        input [LOAD_W-1:0] load_ord_i;
        begin
            load_cache_bank = load_ord_i / ROWS;
        end
    endfunction
    ...
    function [ROW_SEL_W-1:0] load_cache_row;
        input [LOAD_W-1:0] load_ord_i;
        begin
            load_cache_row = load_ord_i % ROWS;
        end
    endfunction
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n || clear) begin
            ...
            if (dw1_fire) begin
                full_idx = dw1_out_t % RING_ROWS;
                slot_valid[dw1_bank_c][full_idx[ROW_IDX_W-1:0]] <= 1'b1;
                slot_tag[dw1_bank_c][full_idx[ROW_IDX_W-1:0]] <= dw1_out_t;

                row_seen_valid[full_idx[ROW_IDX_W-1:0]] <= 1'b1;
                row_seen_tag[full_idx[ROW_IDX_W-1:0]] <= dw1_out_t;
                if (!row_seen_valid[full_idx[ROW_IDX_W-1:0]]
                    || (row_seen_tag[full_idx[ROW_IDX_W-1:0]] != dw1_out_t)) begin
                    row_seen_mask[full_idx[ROW_IDX_W-1:0]]
                        <= bank_onehot(dw1_bank_c);
                end else begin
                    row_seen_mask[full_idx[ROW_IDX_W-1:0]]
                        <= row_seen_mask[full_idx[ROW_IDX_W-1:0]]
                        | bank_onehot(dw1_bank_c);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n || clear) begin
            ...
            bram_a_cache_bank_d[0] <= load_cache_bank(load_issue_idx);
            bram_b_cache_bank_d[0] <= load_cache_bank(load_issue_idx + 1'b1);
            bram_a_cache_row_d[0] <= load_cache_row(load_issue_idx);
            bram_b_cache_row_d[0] <= load_cache_row(load_issue_idx + 1'b1);
            ...
            case (rd_state)
                ...
                RD_READY: begin
                    if (act_req_fire) begin
                        act_resp_valid <= 1'b1;
                        act_resp_last <= (act_req_k == (PW1_IC - 1));
