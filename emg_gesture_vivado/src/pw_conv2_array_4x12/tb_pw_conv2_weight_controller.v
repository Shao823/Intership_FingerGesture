...
module tb_pw_conv2_weight_controller #(
    ...
    localparam integer TIMEOUT    = 3000;
    ...
    initial begin
        $readmemh({`PROJECT_ROOT, "/weight_data/pw2/weight_packed.mem"}, rom_mem);
    end
        ...
        begin
            ...
            if (addr < 0 || addr >= ROM_DEPTH) begin
                ...
                if (seen_addr[addr]) begin
                    $display("ERROR duplicate ROM address in one sequence addr=%0d", addr);
                    errors = errors + 1;
                end else begin
                    seen_addr[addr] = 1'b1;
        ...
        begin
            ...
            if (ready_mode == READY_ALWAYS) begin
                if ((first_req_cycle < 0) || (first_resp_cycle < 0)) begin
                    ...
                    $display("ERROR %0s first response gap got=%0d expected=%0d for BRAM_READ_LATENCY=%0d",
                        label,
            ...
            $display(
                "INFO PW2 L%0d %0s requests=%0d responses=%0d cycles=%0d req_stalls=%0d req_gaps=%0d first_gap=%0d unique_addrs=%0d",
                BRAM_READ_LATENCY,
                label,
                total_requests,
                total_responses,
    ...
    task run_invalid_request_case;
        ...
        begin
            ...
            $display("INFO PW2 invalid_request_case invalid_delta=%0d",
                invalid_after - invalid_before);
        end
    endtask
    ...
    initial begin
        ...
        if (errors == 0) begin
            $display("PASS tb_pw_conv2_weight_controller");
        end else begin
            $display("FAIL tb_pw_conv2_weight_controller errors=%0d", errors);
            $finish;
        end
        $finish;
    end
