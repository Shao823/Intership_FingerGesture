`timescale 1ns / 1ps

`ifndef PROJECT_ROOT
`define PROJECT_ROOT "D:/Intership/ip_repo/emg_accelerator_1_0/src"
`endif

// LightCNN1D stem Conv1d 4x4 output-stationary systolic compute tile.
//
// One start pulse computes 4 time rows x 4 output channels:
//   stem[tile_t_base + row][oc_base + col]
// The downstream stem_to_dw1 writer should use the metadata as the buffer
// address:
//   buffer[tile_t_base + row][oc_base + col] <= out_tile[row][col]
// where:
//   out_tile[(row*OC_LANES + col)*DATA_W +: DATA_W]
//
// Data format:
//   input, streamed conv weights, output: signed INT8 values sign-extended to
//   DATA_W lanes. MAC uses INT32/48 accumulation, then per-output-channel Q15
//   requantization and ReLU. The valid activation range is [-127, 127].
//
// This module does not store the 32x35 stem convolution weights or the input
// activation window. During MAC it requests one 4-lane weight vector and one
// 4-row activation vector per k:
//   weight_vec = {W[oc_base+3][k], ..., W[oc_base+0][k]}
//   act_vec    = {A[tile_t_base+3][k], ..., A[tile_t_base+0][k]}
//
// The weight ROM/controller accepts a request when
// weight_req_valid && weight_req_ready. The activation ping-pong buffer/window
// loader accepts a request when act_req_valid && act_req_ready. Requests for
// the same k may be accepted in different cycles; req_k advances only after
// both sides have accepted that k.
//
// Each response stream is valid/ready. The compute core buffers at most one
// unmatched response from each side and starts one systolic MAC step only when
// both the weight vector and activation vector for the next k are available.
// Each upstream must return exactly one response for every accepted request, in
// request order. Responses must not be reordered.
//
// Quantization parameters are intentionally kept in the compute block:
//   bias_int32[32], multiplier_q15[32], right_shift[32].
// The legacy folded-BN write ports are retained for interface compatibility;
// normal INT8 deployment initializes these arrays from weight_data/stem/*.mem.
//
// Classic systolic dataflow:
//   activation boundary values are row-skewed by shift registers and flow right
//   weight boundary values are column-skewed by shift registers and flow down
//   every PE keeps its own output-stationary accumulator
//
// PE post-processing:
//   acc_bias = acc + bias_int32[oc]
//   q        = round_half_away_from_zero(acc_bias * multiplier_q15[oc],
//                                        right_shift[oc])
//   output   = ReLU(saturate_int8(q))
//
// The output tile uses valid/ready. out_valid remains asserted until the
// downstream writer accepts the tile with out_valid && out_ready.

module stem_conv_array_4x4_compute #(
    parameter integer DATA_W     =  8,
    parameter integer MULT_W     = 16,
    parameter integer ACC_W      = 48,
    parameter integer ROWS       = 4,
    parameter integer OC_LANES   = 4,
    parameter integer STEM_OC    = 32,
    parameter integer STEM_K     = 35,
    parameter integer FRAC_BITS  = 8,
    parameter integer TIME_W     = 9,
    parameter BIAS_INIT_FILE     = {`PROJECT_ROOT, "/weight_data/stem/bias_int32.mem"},
    parameter MULT_INIT_FILE     = {`PROJECT_ROOT, "/weight_data/stem/multiplier_q15.mem"},
    parameter SHIFT_INIT_FILE    = {`PROJECT_ROOT, "/weight_data/stem/right_shift.mem"}
) (
    input  wire clk,
    input  wire rst_n,

    input  wire start,
    input  wire [4:0] oc_base,
    input  wire [TIME_W-1:0] tile_t_base,

    output wire weight_req_valid,
    output wire [4:0] weight_req_oc_base,
    output wire [5:0] weight_req_k,
    input  wire weight_req_ready,
    input  wire weight_vec_valid,
    output wire weight_vec_ready,
    input  wire signed [OC_LANES*DATA_W-1:0] weight_vec,

    output wire act_req_valid,
    output wire [TIME_W-1:0] act_req_t_base,
    output wire [5:0] act_req_k,
    input  wire act_req_ready,
    input  wire act_vec_valid,
    output wire act_vec_ready,
    input  wire signed [ROWS*DATA_W-1:0] act_vec,

    input  wire w_fold_we,
    input  wire [4:0] w_fold_oc,
    input  wire signed [MULT_W-1:0] w_fold_wdata,

    input  wire bias_fold_we,
    input  wire [4:0] bias_fold_oc,
    input  wire signed [31:0] bias_fold_wdata,

    input  wire out_ready,
    output reg busy,
    output reg done,
    output reg out_valid,
    output reg [TIME_W-1:0] out_tile_t_base,
    output reg [4:0] out_tile_oc_base,
    output reg signed [ROWS*OC_LANES*DATA_W-1:0] out_tile
);

    localparam integer FLUSH_CYCLES = ROWS + OC_LANES - 2;
    localparam integer REQ_W        = (STEM_K <= 1) ? 1 : $clog2(STEM_K + 1);
    localparam integer DRAIN_W      = (FLUSH_CYCLES <= 1) ? 1 : $clog2(FLUSH_CYCLES + 1);

    localparam [2:0] STATE_IDLE  = 3'd0;
    localparam [2:0] STATE_MAC   = 3'd1;
    localparam [2:0] STATE_POST0 = 3'd2;
    localparam [2:0] STATE_POST1 = 3'd3;
    localparam [2:0] STATE_POST2 = 3'd4;
    localparam [2:0] STATE_DONE  = 3'd5;

    reg [2:0] state;
    reg [4:0] oc_base_reg;
    reg [TIME_W-1:0] tile_t_base_reg;
    reg [REQ_W-1:0] req_k;
    reg [REQ_W-1:0] recv_k;
    reg [DRAIN_W-1:0] drain_count;
    reg weight_req_sent;
    reg act_req_sent;

    reg weight_hold_valid;
    reg signed [OC_LANES*DATA_W-1:0] weight_hold_data;
    reg act_hold_valid;
    reg signed [ROWS*DATA_W-1:0] act_hold_data;

    reg signed [31:0] bias_int32_mem [0:STEM_OC-1];
    reg signed [MULT_W-1:0] multiplier_q15_mem [0:STEM_OC-1];
    reg [7:0] right_shift_mem [0:STEM_OC-1];

    initial begin
        $readmemh(BIAS_INIT_FILE, bias_int32_mem);
        $readmemh(MULT_INIT_FILE, multiplier_q15_mem);
        $readmemh(SHIFT_INIT_FILE, right_shift_mem);
    end

    reg signed [DATA_W-1:0] a_delay [0:ROWS-1][0:ROWS-1];
    reg signed [DATA_W-1:0] w_delay [0:OC_LANES-1][0:OC_LANES-1];

    wire [4:0] oc_index [0:OC_LANES-1];
    wire signed [DATA_W-1:0] raw_a [0:ROWS-1];
    wire signed [DATA_W-1:0] raw_w [0:OC_LANES-1];
    wire signed [DATA_W-1:0] a_left [0:ROWS-1];
    wire signed [DATA_W-1:0] w_top [0:OC_LANES-1];

    wire signed [DATA_W-1:0] a_bus [0:ROWS-1][0:OC_LANES];
    wire signed [DATA_W-1:0] w_bus [0:ROWS][0:OC_LANES-1];
    wire signed [DATA_W-1:0] post_bus [0:ROWS-1][0:OC_LANES-1];

    wire feed_valid;
    wire flush_valid;
    wire request_pending;
    wire weight_req_fire;
    wire act_req_fire;
    wire request_pair_done;
    wire can_accept_resp;
    wire weight_vec_fire;
    wire act_vec_fire;
    wire weight_avail;
    wire act_avail;
    wire mac_step_en;
    wire pe_clear;
    wire pe_mac_en;
    wire pe_post0_en;
    wire pe_post1_en;
    wire pe_post2_en;
    wire bn_write_en;
    wire signed [OC_LANES*DATA_W-1:0] feed_weight_vec;
    wire signed [ROWS*DATA_W-1:0] feed_act_vec;

    integer i;
    integer j;
    integer s;

    assign request_pending = (state == STATE_MAC) && (req_k < STEM_K);

    assign weight_req_valid = request_pending && !weight_req_sent;
    assign weight_req_oc_base = oc_base_reg;
    assign weight_req_k = req_k[5:0];

    assign act_req_valid = request_pending && !act_req_sent;
    assign act_req_t_base = tile_t_base_reg;
    assign act_req_k = req_k[5:0];

    assign weight_req_fire = weight_req_valid && weight_req_ready;
    assign act_req_fire = act_req_valid && act_req_ready;
    assign request_pair_done = request_pending
        && (weight_req_sent || weight_req_fire)
        && (act_req_sent || act_req_fire);

    assign can_accept_resp = (state == STATE_MAC) && (recv_k < STEM_K);
    assign weight_vec_ready = can_accept_resp && !weight_hold_valid;
    assign act_vec_ready = can_accept_resp && !act_hold_valid;
    assign weight_vec_fire = weight_vec_valid && weight_vec_ready;
    assign act_vec_fire = act_vec_valid && act_vec_ready;
    assign weight_avail = weight_hold_valid || weight_vec_fire;
    assign act_avail = act_hold_valid || act_vec_fire;
    assign feed_valid = can_accept_resp && weight_avail && act_avail;
    assign flush_valid = (state == STATE_MAC) && (recv_k == STEM_K);
    assign mac_step_en = feed_valid || flush_valid;

    assign feed_weight_vec = weight_hold_valid ? weight_hold_data : weight_vec;
    assign feed_act_vec = act_hold_valid ? act_hold_data : act_vec;

    assign pe_clear = (state == STATE_IDLE) && start;
    assign pe_mac_en = mac_step_en;
    assign pe_post0_en = (state == STATE_POST0);
    assign pe_post1_en = (state == STATE_POST1);
    assign pe_post2_en = (state == STATE_POST2);
    assign bn_write_en = (state == STATE_IDLE) && !start;

    genvar gc;
    generate
        for (gc = 0; gc < OC_LANES; gc = gc + 1) begin : GEN_OC_INDEX
            assign oc_index[gc] = oc_base_reg + gc[4:0];
            assign raw_w[gc] = feed_valid
                ? feed_weight_vec[gc*DATA_W +: DATA_W]
                : {DATA_W{1'b0}};
            assign w_top[gc] = (gc == 0) ? raw_w[gc] : w_delay[gc][gc-1];
            assign w_bus[0][gc] = w_top[gc];
        end
    endgenerate

    genvar gr;
    generate
        for (gr = 0; gr < ROWS; gr = gr + 1) begin : GEN_A_LEFT
            assign raw_a[gr] = feed_valid
                ? feed_act_vec[gr*DATA_W +: DATA_W]
                : {DATA_W{1'b0}};
            assign a_left[gr] = (gr == 0) ? raw_a[gr] : a_delay[gr][gr-1];
            assign a_bus[gr][0] = a_left[gr];
        end
    endgenerate

    genvar pr;
    genvar pc;
    generate
        for (pr = 0; pr < ROWS; pr = pr + 1) begin : GEN_PE_ROW
            for (pc = 0; pc < OC_LANES; pc = pc + 1) begin : GEN_PE_COL
                stem_conv_systolic_pe #(
                    .DATA_W(DATA_W),
                    .MULT_W(MULT_W),
                    .ACC_W(ACC_W),
                    .FRAC_BITS(FRAC_BITS)
                ) u_pe (
                    .clk(clk),
                    .rst_n(rst_n),
                    .clear(pe_clear),
                    .mac_en(pe_mac_en),
                    .post0_en(pe_post0_en),
                    .post1_en(pe_post1_en),
                    .post2_en(pe_post2_en),
                    .a_in(a_bus[pr][pc]),
                    .w_in(w_bus[pr][pc]),
                    .bias_int32(bias_int32_mem[oc_index[pc]]),
                    .multiplier_q15(multiplier_q15_mem[oc_index[pc]]),
                    .right_shift(right_shift_mem[oc_index[pc]]),
                    .a_out(a_bus[pr][pc+1]),
                    .w_out(w_bus[pr+1][pc]),
                    .post_out(post_bus[pr][pc])
                );
            end
        end
    endgenerate

    always @(posedge clk) begin
        if (bn_write_en && w_fold_we) begin
            multiplier_q15_mem[w_fold_oc] <= w_fold_wdata;
        end
        if (bn_write_en && bias_fold_we) begin
            bias_int32_mem[bias_fold_oc] <= bias_fold_wdata;
        end
    end

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            weight_hold_valid <= 1'b0;
            weight_hold_data <= {(OC_LANES*DATA_W){1'b0}};
            act_hold_valid <= 1'b0;
            act_hold_data <= {(ROWS*DATA_W){1'b0}};
        end else if (pe_clear || (state != STATE_MAC)) begin
            weight_hold_valid <= 1'b0;
            weight_hold_data <= {(OC_LANES*DATA_W){1'b0}};
            act_hold_valid <= 1'b0;
            act_hold_data <= {(ROWS*DATA_W){1'b0}};
        end else if (feed_valid) begin
            weight_hold_valid <= 1'b0;
            weight_hold_data <= {(OC_LANES*DATA_W){1'b0}};
            act_hold_valid <= 1'b0;
            act_hold_data <= {(ROWS*DATA_W){1'b0}};
        end else begin
            if (weight_vec_fire) begin
                weight_hold_valid <= 1'b1;
                weight_hold_data <= weight_vec;
            end
            if (act_vec_fire) begin
                act_hold_valid <= 1'b1;
                act_hold_data <= act_vec;
            end
        end
    end

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            for (i = 0; i < ROWS; i = i + 1) begin
                for (s = 0; s < ROWS; s = s + 1) begin
                    a_delay[i][s] <= {DATA_W{1'b0}};
                end
            end
            for (j = 0; j < OC_LANES; j = j + 1) begin
                for (s = 0; s < OC_LANES; s = s + 1) begin
                    w_delay[j][s] <= {DATA_W{1'b0}};
                end
            end
        end else if (pe_clear) begin
            for (i = 0; i < ROWS; i = i + 1) begin
                for (s = 0; s < ROWS; s = s + 1) begin
                    a_delay[i][s] <= {DATA_W{1'b0}};
                end
            end
            for (j = 0; j < OC_LANES; j = j + 1) begin
                for (s = 0; s < OC_LANES; s = s + 1) begin
                    w_delay[j][s] <= {DATA_W{1'b0}};
                end
            end
        end else if (mac_step_en) begin
            for (i = 0; i < ROWS; i = i + 1) begin
                a_delay[i][0] <= raw_a[i];
                for (s = 1; s < ROWS; s = s + 1) begin
                    a_delay[i][s] <= a_delay[i][s-1];
                end
            end
            for (j = 0; j < OC_LANES; j = j + 1) begin
                w_delay[j][0] <= raw_w[j];
                for (s = 1; s < OC_LANES; s = s + 1) begin
                    w_delay[j][s] <= w_delay[j][s-1];
                end
            end
        end
    end

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            state <= STATE_IDLE;
            oc_base_reg <= 5'd0;
            tile_t_base_reg <= {TIME_W{1'b0}};
            req_k <= {REQ_W{1'b0}};
            recv_k <= {REQ_W{1'b0}};
            drain_count <= {DRAIN_W{1'b0}};
            weight_req_sent <= 1'b0;
            act_req_sent <= 1'b0;
            busy <= 1'b0;
            done <= 1'b0;
            out_valid <= 1'b0;
            out_tile_t_base <= {TIME_W{1'b0}};
            out_tile_oc_base <= 5'd0;
            out_tile <= {(ROWS*OC_LANES*DATA_W){1'b0}};
        end else begin
            done <= 1'b0;

            case (state)
                STATE_IDLE: begin
                    busy <= 1'b0;
                    out_valid <= 1'b0;
                    req_k <= {REQ_W{1'b0}};
                    recv_k <= {REQ_W{1'b0}};
                    drain_count <= {DRAIN_W{1'b0}};
                    weight_req_sent <= 1'b0;
                    act_req_sent <= 1'b0;
                    if (start) begin
                        busy <= 1'b1;
                        oc_base_reg <= oc_base;
                        tile_t_base_reg <= tile_t_base;
                        state <= STATE_MAC;
                    end
                end

                STATE_MAC: begin
                    if (request_pair_done) begin
                        req_k <= req_k + 1'b1;
                        weight_req_sent <= 1'b0;
                        act_req_sent <= 1'b0;
                    end else begin
                        if (weight_req_fire) begin
                            weight_req_sent <= 1'b1;
                        end
                        if (act_req_fire) begin
                            act_req_sent <= 1'b1;
                        end
                    end

                    if (feed_valid) begin
                        recv_k <= recv_k + 1'b1;
                    end else if (flush_valid) begin
                        if (drain_count == FLUSH_CYCLES-1) begin
                            drain_count <= {DRAIN_W{1'b0}};
                            state <= STATE_POST0;
                        end else begin
                            drain_count <= drain_count + 1'b1;
                        end
                    end
                end

                STATE_POST0: begin
                    state <= STATE_POST1;
                end

                STATE_POST1: begin
                    state <= STATE_POST2;
                end

                STATE_POST2: begin
                    state <= STATE_DONE;
                end

                STATE_DONE: begin
                    busy <= 1'b1;
                    if (!out_valid) begin
                        for (i = 0; i < ROWS; i = i + 1) begin
                            for (j = 0; j < OC_LANES; j = j + 1) begin
                                out_tile[(i*OC_LANES + j)*DATA_W +: DATA_W]
                                    <= post_bus[i][j];
                            end
                        end
                        out_tile_t_base <= tile_t_base_reg;
                        out_tile_oc_base <= oc_base_reg;
                        out_valid <= 1'b1;
                    end else if (out_ready) begin
                        out_valid <= 1'b0;
                        done <= 1'b1;
                        busy <= 1'b0;
                        state <= STATE_IDLE;
                    end
                end

                default: begin
                    state <= STATE_IDLE;
                    busy <= 1'b0;
                    out_valid <= 1'b0;
                end
            endcase
        end
    end

endmodule

module stem_conv_systolic_pe #(
    parameter integer DATA_W    =  8,
    parameter integer MULT_W    = 16,
    parameter integer ACC_W     = 48,
    parameter integer FRAC_BITS = 8
) (
    input  wire clk,
    input  wire rst_n,
    input  wire clear,
    input  wire mac_en,
    input  wire post0_en,
    input  wire post1_en,
    input  wire post2_en,

    input  wire signed [DATA_W-1:0] a_in,
    input  wire signed [DATA_W-1:0] w_in,
    input  wire signed [31:0] bias_int32,
    input  wire signed [MULT_W-1:0] multiplier_q15,
    input  wire [7:0] right_shift,

    output reg signed [DATA_W-1:0] a_out,
    output reg signed [DATA_W-1:0] w_out,
    output reg signed [DATA_W-1:0] post_out
);

    reg signed [ACC_W-1:0] acc;
    (* use_dsp = "yes" *) wire signed [31:0] mul_p;
    reg signed [ACC_W-1:0] acc_with_bias;
    reg signed [63:0] requant_product;
    reg [7:0] requant_shift;

    assign mul_p = $signed(a_in) * $signed(w_in);

    function signed [DATA_W-1:0] saturate_int8;
        input signed [ACC_W-1:0] value;
        begin
            if (value > 127) begin
                saturate_int8 = 8'sh7F;
            end else if (value < -127) begin
                saturate_int8 = -8'sd127;
            end else begin
                saturate_int8 = value[DATA_W-1:0];
            end
        end
    endfunction

    function signed [ACC_W-1:0] requant_product_q15;
        input signed [63:0] product;
        input [7:0] shift;
        reg [63:0] magnitude;
        reg [63:0] rounded_mag;
        reg [63:0] round_offset;
        reg signed [63:0] rounded;
        begin
            magnitude = product[63] ? (~product + 64'd1) : product;
            round_offset = (shift == 0) ? 64'd0 : (64'd1 << (shift - 1));
            rounded_mag = (magnitude + round_offset) >> shift;
            rounded = product[63] ? -$signed(rounded_mag) : $signed(rounded_mag);
            requant_product_q15 = rounded[ACC_W-1:0];
        end
    endfunction

    function signed [DATA_W-1:0] relu_int8;
        input signed [DATA_W-1:0] value;
        begin
            if (value[DATA_W-1]) begin
                relu_int8 = {DATA_W{1'b0}};
            end else begin
                relu_int8 = value;
            end
        end
    endfunction

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            acc <= {ACC_W{1'b0}};
            acc_with_bias <= {ACC_W{1'b0}};
            requant_product <= 64'sd0;
            requant_shift <= 8'd0;
            a_out <= {DATA_W{1'b0}};
            w_out <= {DATA_W{1'b0}};
            post_out <= {DATA_W{1'b0}};
        end else if (clear) begin
            acc <= {ACC_W{1'b0}};
            acc_with_bias <= {ACC_W{1'b0}};
            requant_product <= 64'sd0;
            requant_shift <= 8'd0;
            a_out <= {DATA_W{1'b0}};
            w_out <= {DATA_W{1'b0}};
            post_out <= {DATA_W{1'b0}};
        end else begin
            if (mac_en) begin
                a_out <= a_in;
                w_out <= w_in;
                acc <= acc + $signed(mul_p);
            end

            // During POST0/POST1 the systolic data registers intentionally hold
            // their last MAC values. Only acc/bn_mul/post_out are meaningful.
            if (post0_en) begin
                acc_with_bias <= acc + $signed({{(ACC_W-32){bias_int32[31]}}, bias_int32});
            end

            if (post1_en) begin
                requant_product <= $signed(acc_with_bias) * $signed(multiplier_q15);
                requant_shift <= right_shift;
            end

            if (post2_en) begin
                post_out <= relu_int8(
                    saturate_int8(
                        requant_product_q15(requant_product, requant_shift)
                    )
                );
            end
        end
    end

endmodule
