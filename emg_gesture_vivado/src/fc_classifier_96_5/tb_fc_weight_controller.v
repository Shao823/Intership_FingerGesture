...
module tb_fc_weight_controller;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (rom_en_d) begin
                rom_dout <= packed_mem[rom_addr_d];
            end
            rom_en_d <= rom_en;
        end
    end
    ...
    task request_and_check;
        ...
        begin
            ...
            if (weight_vec_valid !== 1'b1) begin
                $display(
                    "ERROR fc_weight_controller timeout layer=%0d oc_base=%0d k=%0d",
                    layer_i,
                    oc_base_i,
                    k_i_local
                );
                ...
                if (weight_vec !== expected_word) begin
                    $display(
                        "ERROR fc_weight_controller word layer=%0d oc_base=%0d k=%0d addr=%0d got=%h expected=%h",
                        layer_i,
                        oc_base_i,
                        k_i_local,
                        expected_addr,
    ...
    initial begin
        ...
        if (errors == 0) begin
            $display("PASS fc_weight_controller self-check");
            $finish;
        end else begin
            $display("FAIL fc_weight_controller errors=%0d", errors);
            $fatal(1);
        end
    end
