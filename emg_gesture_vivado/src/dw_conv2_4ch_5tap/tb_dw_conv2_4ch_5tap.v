...
module tb_dw_conv2_4ch_5tap;
    ...
    initial begin
        $readmemh(
            {`PROJECT_ROOT, "/emg_gesture_vivado/src/dw_conv2_4ch_5tap/testdata/input.mem"},
            file_input
        );
        $readmemh(
            {`PROJECT_ROOT, "/emg_gesture_vivado/src/dw_conv2_4ch_5tap/testdata/weight.mem"},
            file_weight
        );
        $readmemh(
            {`PROJECT_ROOT, "/emg_gesture_vivado/src/dw_conv2_4ch_5tap/testdata/expected_full.mem"},
            file_expected_full
        );
        $readmemh(
            {`PROJECT_ROOT, "/emg_gesture_vivado/src/dw_conv2_4ch_5tap/testdata/case_ch_base.mem"},
            file_case_ch_base
        );
        $readmemh(
            {`PROJECT_ROOT, "/emg_gesture_vivado/src/dw_conv2_4ch_5tap/testdata/case_input_t_base.mem"},
            file_case_input_t_base
        );
        $readmemh(
            {`PROJECT_ROOT, "/emg_gesture_vivado/src/dw_conv2_4ch_5tap/testdata/case_input_rows.mem"},
            file_case_input_rows
        );
        $readmemh(
            {`PROJECT_ROOT, "/emg_gesture_vivado/src/dw_conv2_4ch_5tap/testdata/case_final_chunk.mem"},
    ...
    function signed [DATA_W-1:0] input_value;
        ...
        begin
            if (mode == FILE_MODE) begin
                ...
                input_value = q(1536 - (ch_idx % 8) * 64 + (t_idx % 7) * 24);
            end else begin
                input_value = q(((ch_idx % 11) - 5) * 18 + (t_idx - 8) * 10);
            end
        end
    endfunction
    ...
    function signed [DATA_W-1:0] weight_value;
        ...
        begin
            ...
            end else if ((ch_idx >= 60) && (tap_idx == 2)) begin
                weight_value = q(1024);
            end else begin
                weight_value = q(((ch_idx % 7) - 3) * 12 + (tap_idx - 2) * 24);
            end
        end
    endfunction
    ...
    function integer expected_stream_count;
        input integer per_group_count;
        begin
            expected_stream_count = per_group_count * CH_GROUPS_PER_JOB;
        end
    endfunction
    ...
    task check_window_request;
        integer expected_req_group;
        integer expected_req_idx;
        integer expected_req_t;
        integer expected_req_ch_base;
        begin
            if (test_expected_count == 0) begin
                $display(
                    "ERROR DW2 unexpected win_req for empty job t=%0d ch_base=%0d",
                    win_req_out_t,
                    win_req_ch_base
                );
                errors = errors + 1;
            end else begin
                expected_req_group = win_req_count / test_expected_count;
                expected_req_idx = win_req_count % test_expected_count;
                expected_req_t = test_expected_start + expected_req_idx;
                expected_req_ch_base = test_ch_base + expected_req_group * P_CH;

                if (expected_req_group >= CH_GROUPS_PER_JOB) begin
                    $display(
                        "ERROR DW2 extra win_req group=%0d req_count=%0d",
                        expected_req_group,
                        win_req_count
                    );
                    errors = errors + 1;
                ...
                if (win_req_out_t !== expected_req_t[TIME_W-1:0]) begin
                    $display(
                        "ERROR DW2 win_req_out_t order got=%0d expected=%0d req_count=%0d",
                        win_req_out_t,
                        expected_req_t,
                        win_req_count
                    );
                ...
                if (win_req_ch_base !== expected_req_ch_base[5:0]) begin
                    $display(
                        "ERROR DW2 win_req_ch_base got=%0d expected=%0d req_count=%0d",
                        win_req_ch_base,
                        expected_req_ch_base,
                        win_req_count
                    );
                    errors = errors + 1;
                end
    ...
    task check_output_row;
        ...
        integer expected_t;
        integer expected_ch_base;
        begin
            if (test_expected_count == 0) begin
                $display(
                    "ERROR DW2 unexpected output for empty job t=%0d ch_base=%0d",
                    out_t,
                    out_ch_base
                );
                errors = errors + 1;
            end else begin
                expected_group = output_count / test_expected_count;
                expected_idx = output_count % test_expected_count;
                expected_t = test_expected_start + expected_idx;
                expected_ch_base = test_ch_base + expected_group * P_CH;

                if (expected_group >= CH_GROUPS_PER_JOB) begin
                    $display(
                        "ERROR DW2 extra output group=%0d output_count=%0d",
                        expected_group,
                        output_count
                    );
                    errors = errors + 1;
                ...
                if (out_t !== expected_t[TIME_W-1:0]) begin
                    $display("ERROR DW2 out_t got=%0d expected=%0d", out_t, expected_t);
                    errors = errors + 1;
                end

                if (out_ch_base !== expected_ch_base[5:0]) begin
                    $display(
                        "ERROR DW2 out_ch_base got=%0d expected=%0d output_count=%0d",
                        out_ch_base,
                        expected_ch_base,
                        output_count
                    );
                ...
                for (lane = 0; lane < P_CH; lane = lane + 1) begin
                    expected = expected_value(
                        test_mode,
                        expected_ch_base + lane,
                        expected_t
                    );
                    if (get_out_lane(lane) !== expected) begin
                        $display(
                            "ERROR DW2 t=%0d lane=%0d ch=%0d got=%0d expected=%0d",
                            expected_t,
                            lane,
                            expected_ch_base + lane,
    ...
    task wait_done;
        ...
        begin
            ...
            if (done !== 1'b1) begin
                $display(
                    "ERROR DW2 timeout mode=%0d ch_base=%0d input_base=%0d final=%0d",
                    test_mode,
                    test_ch_base,
                    test_input_t_base,
                    test_final_chunk
    ...
    task run_checked_ext;
        ...
        begin
            ...
            if (job_out_t_start !== test_expected_start[TIME_W-1:0]) begin
                $display(
                    "ERROR DW2 job_out_t_start got=%0d expected=%0d",
                    job_out_t_start,
                    test_expected_start
                );
                errors = errors + 1;
            ...
            if (job_out_count !== test_expected_count[TIME_W:0]) begin
                $display(
                    "ERROR DW2 job_out_count got=%0d expected=%0d",
                    job_out_count,
                    test_expected_count
                );
                errors = errors + 1;
            ...
            wait_done();
            if (!last_timeout
                && (win_req_count != expected_stream_count(test_expected_count))) begin
                $display(
                    "ERROR DW2 win_req_count got=%0d expected=%0d",
                    win_req_count,
                    expected_stream_count(test_expected_count)
                );
                errors = errors + 1;
            end
            if (!last_timeout
                && (output_count != expected_stream_count(test_expected_count))) begin
                $display(
                    "ERROR DW2 output_count got=%0d expected=%0d",
                    output_count,
                    expected_stream_count(test_expected_count)
                );
                errors = errors + 1;
    ...
    task run_with_busy_start_pulse;
        begin
            ...
            if (!last_timeout
                && (win_req_count != expected_stream_count(test_expected_count))) begin
                $display(
                    "ERROR DW2 busy-start win_req_count got=%0d expected=%0d",
                    win_req_count,
                    expected_stream_count(test_expected_count)
                );
                errors = errors + 1;
            ...
            if (!last_timeout
                && (output_count != expected_stream_count(test_expected_count))) begin
                $display(
                    "ERROR DW2 busy-start output_count got=%0d expected=%0d",
                    output_count,
                    expected_stream_count(test_expected_count)
                );
                errors = errors + 1;
    ...
    task run_with_busy_weight_write;
        begin
            ...
            if (!last_timeout
                && (win_req_count != expected_stream_count(test_expected_count))) begin
                $display(
                    "ERROR DW2 busy-weight win_req_count got=%0d expected=%0d",
                    win_req_count,
                    expected_stream_count(test_expected_count)
                );
                errors = errors + 1;
            ...
            if (!last_timeout
                && (output_count != expected_stream_count(test_expected_count))) begin
                $display(
                    "ERROR DW2 busy-weight output_count got=%0d expected=%0d",
                    output_count,
                    expected_stream_count(test_expected_count)
                );
                errors = errors + 1;
    ...
    task reset_during_busy;
        begin
            ...
            #1;
            if (busy !== 1'b0) begin
                $display("ERROR busy is not low after DW2 mid-run reset");
                errors = errors + 1;
            end
            if (out_valid !== 1'b0) begin
                $display("ERROR out_valid is high after DW2 mid-run reset");
                errors = errors + 1;
            end
    ...
    initial begin
        ...
        if (errors == 0) begin
            $display("PASS dw_conv2_4ch_5tap self-check");
            $finish;
        end else begin
            $display("FAIL dw_conv2_4ch_5tap errors=%0d", errors);
            $finish;
        end
    end
