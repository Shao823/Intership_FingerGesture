...
module tb_pw_conv1_weight_controller_with_rom;
    ...
    localparam integer ROM_DEPTH  = PW1_OC / OC_LANES * PW1_IC;
    localparam integer ADDR_W     = (ROM_DEPTH <= 1) ? 1 : $clog2(ROM_DEPTH);
    localparam integer MAX_REQS   = 512;
    localparam integer TIMEOUT    = 2500;
    // This TB instantiates the generated blk_mem_gen_pw_conv1_weight IP, whose
    ...
    initial begin
        clk = 1'b0;
        $readmemh({`PROJECT_ROOT, "/weight_data/pw1/weight_packed.mem"}, expected_mem);
    end
    ...
    function integer calc_addr;
        ...
        input [5:0] k_i;
        begin
            calc_addr = (oc_base_i / OC_LANES) * PW1_IC + k_i;
        end
    endfunction

    function [5:0] req_oc_for_index;
        input integer idx;
        integer group;
        begin
            group = (idx / PW1_IC) % (PW1_OC / OC_LANES);
    ...
    task check_expected_mem_loaded;
        begin
            if ((^expected_mem[0]) === 1'bx) begin
                $display("ERROR PW1 expected weight_packed.mem did not load");
                errors = errors + 1;
            end
        end
    endtask
    ...
    task run_autoload_probe;
        ...
        begin
            ...
            while (!responded && (probe_timeout < 80)) begin
                ...
                if (weight_vec_valid && weight_vec_ready) begin
                    ...
                    if (weight_vec !== expected_mem[0]) begin
                        $display(
                            "ERROR PW1 ROM autoload probe mismatch got=%h expected=%h",
                            weight_vec,
                            expected_mem[0]
                        );
                        errors = errors + 1;
            ...
            if (!accepted) begin
                $display("ERROR PW1 ROM autoload probe request was not accepted");
                errors = errors + 1;
            end
            if (!responded) begin
                $display("ERROR PW1 ROM autoload probe did not receive a response");
                errors = errors + 1;
            end

            @(negedge clk);
            weight_vec_ready = 1'b1;
            $display("INFO IP autoload_probe complete");
        end
    endtask
    ...
    task run_invalid_request_case;
        ...
        begin
            ...
            send_invalid_request(6'd5, 6'd0);
            send_invalid_request(6'd0, 6'd32);
    ...
    initial begin
        ...
        run_sequence(
            ...
            1,
            0,
            "all_256_words_periodic_backpressure"
        );
        ...
        if (errors == 0) begin
            $display("PASS tb_pw_conv1_weight_controller_with_rom");
        end else begin
            $display("FAIL tb_pw_conv1_weight_controller_with_rom errors=%0d", errors);
            $finish;
        end
        $finish;
    end
