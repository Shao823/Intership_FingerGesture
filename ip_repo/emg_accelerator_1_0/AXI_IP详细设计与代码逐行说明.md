...
5      parameter integer C_S00_AXI_ADDR_WIDTH = 5,
...
### 4.13 第 144～188 行：CONTROL 真正写入位置
...
```verilog
if (slv_reg_wren && write_index == 0 && S_AXI_WSTRB[0])
```

只有 `0x00 CONTROL`、低 byte strobe 有效时才执行用户写动作。
...
持久保存，直到下次 CONTROL 写或复位。

#### 第 161～163 行 CLEAR_DONE

`WDATA[2]=1` 清除 `done_sticky`。

#### 第 164 行 CLEAR_ERRORS

`WDATA[3]` 形成一拍 `clear_errors_pulse`。

#### 第 165 行 IRQ_ENABLE

`irq_enable <= WDATA[4]`，持久保存。

#### 第 166～169 行

CLEAR_ERRORS 同时清除两个 start reject sticky bit。
...
每次 CONTROL 写都会覆盖 `auto_start` 和 `irq_enable`。例如只写 `0x1` 启动，会同时把 AUTO_START 和 IRQ_ENABLE 写成 0。

软件应写完整控制字，或先读 CONTROL 后做 read-modify-write。
...
CPU 写 `CONTROL=0x11`，即 START 和 IRQ_ENABLE：

```text
周期 A：master 拉高 AWVALID、WVALID，给出 AWADDR=0、WDATA=0x11
周期 B：slave 拉高 AWREADY、WREADY，完成地址和数据握手
同一上升沿：slv_reg_wren=1，执行 CONTROL 写入，start_pulse=1
同一上升沿：BVALID=1，BRESP=OKAY
...
当前 AXI-Lite 写地址和写数据要求同时有效。后续可分别增加地址/data hold register，实现完全独立通道接收。

### 17.2 只有 CONTROL 可写

写其他地址不改变状态，但仍返回 OKAY。
...
2. 启动 DMA MM2S，length=1740 byte
3. 等待 STATUS.FRAME_READY
4. 写 CONTROL.START，并保留 IRQ_ENABLE/AUTO_START 期望值
5. 等待 STATUS.DONE 或 irq
6. 读取 RESULT
7. 读取 CYCLE_COUNT
8. 写 CLEAR_DONE 清 irq
```
...
```text
1. 设置 AUTO_START
2. 每次提交 1740-byte DMA frame
3. frame_ready 且 core idle 时自动启动
4. 通过 DONE/irq 读取结果
```
