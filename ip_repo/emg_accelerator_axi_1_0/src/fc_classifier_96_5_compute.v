        ...
        if (!rst_n) begin
            ...
            case (state)
                    ...
                    for (j = 0; j < AVG_LANES; j = j + 1) begin
                        ...
                        if (avg_ch < FC0_IN) begin
                            avg_mem[avg_ch] <= saturate_int8(
                                requant_q15(
                                    avg_gap_group[j],
                                    gap_multiplier_q15_mem[0],
                                    gap_right_shift_mem[0]
                                )
                            );
                        end
                    end
                ...
                STATE_FC0_BIAS_REQ: begin
                    ...
                    for (j = 0; j < FC0_OUT_LANES; j = j + 1) begin
                        if ((fc0_base_reg + j) < FC0_OUT) begin
                            fc0_acc[j] <= $signed({
                                {(ACC_W-32){fc0_bias_int32_mem[fc0_base_reg + j][31]}},
                                fc0_bias_int32_mem[fc0_base_reg + j]
                            });
                        end else begin
                ...
                STATE_FC0_STORE: begin
                    ...
                    for (j = 0; j < FC0_OUT_LANES; j = j + 1) begin
                        if ((fc0_base_reg + j) < FC0_OUT) begin
                            hidden_mem[fc0_base_reg + j] <= relu_int8(
                                saturate_int8(
                                    requant_q15(
                                        fc0_acc[j],
                                        fc0_multiplier_q15_mem[fc0_base_reg + j],
                                        fc0_right_shift_mem[fc0_base_reg + j]
                                    )
                                )
                            );
                        end
                ...
                STATE_FC1_STORE: begin
                    ...
                    for (j = 0; j < FC1_OUT; j = j + 1) begin
                        logit_mem[j] <= saturate_int8(
                            requant_q15(
                                fc1_acc[j],
                                fc1_multiplier_q15_mem[j],
                                fc1_right_shift_mem[j]
                            )
                        );
                    end
                    state <= STATE_ARGMAX;
