...
// Integrated stem -> DW1 -> PW1 pipeline top.
//
...
module stem_dw1_pw1_pipeline_top #(
    ...
    output wire [TIME_W-1:0] pw1_out_pool_t_base,
    output wire [5:0] pw1_out_oc_base,
    output wire pw1_out_last_group,
    output wire signed [POOL_ROWS*PW1_OC_LANES*DATA_W-1:0] pw1_out_tile,
    ...
    initial begin
        if ((STEM_OC % STEM_OC_LANES) != 0) begin
            $error("stem_dw1_pw1_pipeline_top: STEM_OC must be divisible by STEM_OC_LANES");
        end
        if ((PW1_OC % PW1_OC_LANES) != 0) begin
            $error("stem_dw1_pw1_pipeline_top: PW1_OC must be divisible by PW1_OC_LANES");
        end
        if (STEM_OC != 32) begin
            $error("stem_dw1_pw1_pipeline_top: PW1 input channel count assumes STEM_OC=32");
        end
        if (PENDING_DEPTH < 2) begin
            $error("stem_dw1_pw1_pipeline_top: PENDING_DEPTH must be >= 2");
        end
        if (PENDING_DEPTH <= STEM_TO_DW1_SLICE_DEPTH) begin
            $error("stem_dw1_pw1_pipeline_top: PENDING_DEPTH must exceed stem output slice depth");
        end
    end
