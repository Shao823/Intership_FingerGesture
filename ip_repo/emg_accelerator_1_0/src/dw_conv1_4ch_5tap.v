...
//
// The output side is a registered valid/ready stage. The 5-tap datapath is split
// into an II=1 valid pipeline:
//   s0: latch window samples and selected weights
//   s1: multiply
//   s2: reduce 5 products and add INT32 bias
//   s3: Q15 multiply
//   output cycle: round and saturate to INT8 from the held s3 product
// If the downstream output is not ready, s3 holds and back-pressures the window
// response channel. Top-level stream slices provide cross-module elasticity.
...
module dw_conv1_4ch_5tap #(
    ...
    parameter BIAS_INIT_FILE     = {`PROJECT_ROOT, "/weight_data/dw1/bias_int32.mem"},
    parameter MULT_INIT_FILE     = {`PROJECT_ROOT, "/weight_data/dw1/multiplier_q15.mem"},
    parameter SHIFT_INIT_FILE    = {`PROJECT_ROOT, "/weight_data/dw1/right_shift.mem"}
) (
    input  wire clk,
    input  wire rst_n,
    ...
    reg signed [31:0] bias_int32_mem [0:CHANNELS-1];
    reg signed [MULT_W-1:0] multiplier_q15_mem [0:CHANNELS-1];
    reg [7:0] right_shift_mem [0:CHANNELS-1];
    wire pipe_advance;
    ...
    reg [TIME_W-1:0] s2_t;
    reg [4:0] s2_ch_base;
    reg signed [ACC_W-1:0] s2_acc [0:P_CH-1];
    reg signed [MULT_W-1:0] s2_multiplier [0:P_CH-1];
    reg [7:0] s2_shift [0:P_CH-1];

    reg s3_valid;
    reg [TIME_W-1:0] s3_t;
    reg [4:0] s3_ch_base;
    reg signed [63:0] s3_product [0:P_CH-1];
    reg [7:0] s3_shift [0:P_CH-1];
    reg signed [P_CH*DATA_W-1:0] s3_vec_c;
    ...
    assign pipe_advance = !s3_valid || out_ready;
    ...
    assign out_t = s3_t;
    assign out_ch_base = s3_ch_base;
    assign out_vec = s3_vec_c;
    ...
    always @* begin
        s3_vec_c = {(P_CH*DATA_W){1'b0}};
        for (quant_lane = 0; quant_lane < P_CH; quant_lane = quant_lane + 1) begin
            s3_vec_c[quant_lane*DATA_W +: DATA_W] = saturate_int8(
                requant_product_q15(
                    s3_product[quant_lane],
                    s3_shift[quant_lane]
                )
            );
        end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            for (pipe_lane = 0; pipe_lane < P_CH; pipe_lane = pipe_lane + 1) begin
                s2_acc[pipe_lane] <= {ACC_W{1'b0}};
                s2_multiplier[pipe_lane] <= {MULT_W{1'b0}};
                s2_shift[pipe_lane] <= 8'd0;
                s3_product[pipe_lane] <= 64'sd0;
                s3_shift[pipe_lane] <= 8'd0;
            end
            busy <= 1'b0;
            done <= 1'b0;
            ...
            case (state)
                ...
                STATE_RUN: begin
                    ...
                    if (pipe_advance) begin
                        s3_valid <= s2_valid;
                        s3_t <= s2_t;
                        s3_ch_base <= s2_ch_base;
                        for (pipe_lane = 0; pipe_lane < P_CH; pipe_lane = pipe_lane + 1) begin
                            s3_product[pipe_lane]
                                <= $signed(s2_acc[pipe_lane])
                                 * $signed(s2_multiplier[pipe_lane]);
                            s3_shift[pipe_lane] <= s2_shift[pipe_lane];
                        end
                        ...
                        for (pipe_lane = 0; pipe_lane < P_CH; pipe_lane = pipe_lane + 1) begin
                            ...
                            pipe_acc = pipe_acc + $signed({
                                {(ACC_W-32){bias_int32_mem[s1_ch_base + pipe_lane][31]}},
                                bias_int32_mem[s1_ch_base + pipe_lane]
                            });
                            s2_acc[pipe_lane] <= pipe_acc;
                            s2_multiplier[pipe_lane]
                                <= multiplier_q15_mem[s1_ch_base + pipe_lane];
                            s2_shift[pipe_lane]
                                <= right_shift_mem[s1_ch_base + pipe_lane];
                        end
