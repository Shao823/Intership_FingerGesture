...
module tb_stem_activation_controller #(
    ...
    initial begin
        ...
        for (init_addr = 0; init_addr < RAM_DEPTH; init_addr = init_addr + 1) begin
            ram_mem[init_addr] = {WORD_W{1'b0}};
        end

        for (init_ch = 0; init_ch < STEM_IC; init_ch = init_ch + 1) begin
            for (init_word = 0; init_word < TIME_WORDS; init_word = init_word + 1) begin
                init_addr = init_ch * TIME_WORDS + init_word;
                for (init_lane = 0; init_lane < ROWS; init_lane = init_lane + 1) begin
                    init_t = init_word * ROWS + init_lane;
                    if (init_t < INPUT_LEN) begin
                        ram_mem[init_addr][init_lane*DATA_W +: DATA_W]
                            = raw_input[init_t*STEM_IC + init_ch];
                    end
                end
            end
        end
    ...
    task pop_and_check_response;
        begin
            if (exp_count <= 0) begin
                $display("ERROR activation response with empty expected queue");
                errors = errors + 1;
            end else begin
                if (act_vec !== expected_queue[exp_head]) begin
                    $display(
                        "ERROR activation vec mismatch response=%0d got=%h expected=%h",
                        total_responses,
                        act_vec,
    ...
    task run_sequence;
        ...
        begin
            ...
            $display(
                "INFO ACT L%0d %0s requests=%0d responses=%0d cycles=%0d req_stalls=%0d req_gaps=%0d read_issues=%0d unique_reads=%0d",
                BRAM_READ_LATENCY,
                label,
                total_requests,
                total_responses,
    ...
    task send_invalid_request;
        ...
        begin
            ...
            if (act_req_ready !== 1'b1) begin
                $display("ERROR invalid activation request was not accepted");
                errors = errors + 1;
            end
            @(negedge clk);
            act_req_valid = 1'b0;
            ...
            if (act_vec_valid === 1'b1) begin
                ...
                $display("ERROR timeout waiting for invalid activation response");
                errors = errors + 1;
            end
        end
    endtask
    ...
    task run_invalid_request_case;
        ...
        begin
            ...
            if ((invalid_after - invalid_before) != 2) begin
                $display(
                    "ERROR activation invalid_req_count delta got=%0d expected=2",
                    invalid_after - invalid_before
                );
                errors = errors + 1;
            end
            $display("INFO ACT invalid_request_case invalid_delta=%0d", invalid_after - invalid_before);
        end
    endtask
    ...
    initial begin
        ...
        if (errors == 0) begin
            $display("PASS tb_stem_activation_controller");
        end else begin
            $display("FAIL tb_stem_activation_controller errors=%0d", errors);
            $finish;
        end
        $finish;
    end
