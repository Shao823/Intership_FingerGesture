...
// Integrated stem -> DW1 -> PW1 -> DW2 -> PW2 -> FC pipeline top.
//
// One start pulse runs the full 348-sample stream. Stem and DW1 reuse the
// existing pending-job FIFO from stem_dw1_pipeline_top. DW1 output rows are
// written into dw1_to_pw1_buffer; PW1 starts monotonic 4-row tiles
// tile_t_base=0,4,...,344 and waits inside its input-tile handshake until the
// required four DW1 rows are fully present. PW1 pooled output is written to the
// low-resolution PW1-to-DW2 ring buffer, DW2 jobs are queued from accepted PW1
// tiles, and PW2 consumes monotonic low-resolution tiles while accumulating GAP.

module stem_dw1_pw1_dw2_pw2_pipeline_top #(
    ...
    parameter integer PW1_GROUPS = PW1_OC / PW1_OC_LANES,
    parameter integer PW1_TILE_COUNT = (INPUT_LEN + ROWS - 1) / ROWS,
    parameter integer PW2_TILE_COUNT = (POOL_LEN + ROWS - 1) / ROWS,
    ...
    input  wire dw2_weight_we,
    input  wire [5:0] dw2_weight_ch,
    input  wire [2:0] dw2_weight_tap,
    input  wire signed [DATA_W-1:0] dw2_weight_wdata,

    input  wire pw2_w_fold_we,
    input  wire [6:0] pw2_w_fold_oc,
    input  wire signed [DATA_W-1:0] pw2_w_fold_wdata,
    input  wire pw2_bias_fold_we,
    input  wire [6:0] pw2_bias_fold_oc,
    input  wire signed [DATA_W-1:0] pw2_bias_fold_wdata,
    ...
    output wire signed [FC_CLASSES*DATA_W-1:0] fc_logits_vec,

    output wire stem_busy,
    output wire dw1_busy,
    output wire pw1_busy,
    output wire dw2_busy,
    output wire pw2_busy,
    output wire stem_to_dw1_wr_busy,
    output wire stem_to_dw1_rd_busy,
    output wire stem_to_dw1_watermark_stall,
    output wire dw1_to_pw1_wr_busy,
    output wire dw1_to_pw1_rd_busy,
    output wire dw1_to_pw1_tile_cache_valid,
    output wire dw1_to_pw1_overwrite_stall,
    output wire pw1_to_dw2_wr_busy,
    output wire pw1_to_dw2_rd_busy,
    output wire pw1_to_dw2_overwrite_stall,
    output wire dw2_to_pw2_wr_busy,
    output wire dw2_to_pw2_rd_busy,
    output wire dw2_to_pw2_tile_cache_valid,
    output wire dw2_to_pw2_overwrite_stall,
    output wire stem_output_stall,
    output wire dw1_output_stall,
    output wire dw2_output_stall,
    output wire pw1_input_wait,
    output wire pw1_output_stall,
    output wire pw2_input_wait,
    output wire scheduler_dw1_pending_full,
    output wire scheduler_dw2_pending_full,
    output wire [PENDING_CNT_W-1:0] dw1_pending_count_dbg,
    output wire [DW2_PENDING_CNT_W-1:0] dw2_pending_count_dbg,
    ...
    output reg [31:0] pw1_tiles_started,
    output reg [31:0] pw1_tiles_done,
    output reg [31:0] pw1_output_tiles,
    output reg [31:0] dw2_jobs_started,
    output reg [31:0] dw2_jobs_done,
    output reg [31:0] dw2_rows_out,
    output reg [31:0] pw2_tiles_started,
    output reg [31:0] pw2_tiles_done,
    ...
    output reg [31:0] dw2_output_stall_cycles,
    output reg [31:0] stem_to_dw1_watermark_stall_cycles,
    output reg [31:0] dw1_to_pw1_overwrite_stall_cycles,
    output reg [31:0] pw1_to_dw2_overwrite_stall_cycles,
    output reg [31:0] dw2_to_pw2_overwrite_stall_cycles,
    output reg [31:0] pw1_input_wait_cycles,
    output reg [31:0] pw1_output_stall_cycles,
    output reg [31:0] pw2_input_wait_cycles,
    output reg [PENDING_CNT_W-1:0] max_dw1_pending_count,
    output reg [DW2_PENDING_CNT_W-1:0] max_dw2_pending_count
);
    ...
    localparam [DW2_PENDING_CNT_W-1:0] DW2_PENDING_DEPTH_COUNT = DW2_PENDING_DEPTH;
    ...
    wire [LOW_TIME_W-1:0] dw2_job_out_t_start;
    wire [LOW_TIME_W:0] dw2_job_out_count;
    ...
    wire pw2_done;
    wire pw2_gap_frame_done_wire;

    reg fc_start;
    reg fc_started;
    wire fc_done_wire;
    wire fc_busy_wire;
    wire fc_result_valid_wire;
    wire [2:0] fc_class_idx_wire;
    wire fc_input_count_error_wire;
    wire signed [FC_CLASSES*DATA_W-1:0] fc_logits_vec_wire;
    ...
    wire front_complete_now;
    wire dw1_mid_out_fire;
    wire pw1_out_fire;
    wire dw2_mid_out_fire;
    ...
    assign scheduler_dw1_pending_full = busy && !dw1_pending_launch_room;
    assign dw1_pending_count_dbg = dw1_pending_count;
    ...
    assign scheduler_dw2_pending_full = busy && !dw2_pending_room;
    assign dw2_pending_count_dbg = dw2_pending_count;
    assign gap_frame_done = pw2_gap_frame_done_wire;
    assign fc_busy = fc_busy_wire;
    assign fc_result_valid = fc_result_valid_wire;
    assign fc_class_idx = fc_class_idx_wire;
    assign fc_input_count_error = fc_input_count_error_wire;
    assign fc_logits_vec = fc_logits_vec_wire;
    ...
    assign pw1_start_now = busy
        && !buffer_clear
        && !all_pw1_issued
        && !pw1_busy
        && !pw1_start;

    assign dw2_start_now = busy
        && !dw2_pending_empty
        && !dw2_busy
        && !dw2_start;

    assign pw2_start_now = busy
        && !buffer_clear
        && !pw2_gap_clear
        && !all_pw2_issued
        && !pw2_busy
    ...
    assign dw1_pending_push = busy && stem_done;
    assign dw1_pending_pop = dw1_start_now;
    assign dw1_pending_count_next = dw1_pending_count
        + {{(PENDING_CNT_W-1){1'b0}}, dw1_pending_push}
        - {{(PENDING_CNT_W-1){1'b0}}, dw1_pending_pop};

    assign pw1_to_dw2_in_valid = pw1_out_valid && dw2_pending_room;
    assign pw1_out_ready = pw1_to_dw2_in_ready && dw2_pending_room;
    assign dw2_pending_push = pw1_out_fire;
    ...
    assign dw1_mid_out_fire = dw1_mid_out_valid && dw1_mid_out_ready;
    assign pw1_out_fire = pw1_out_valid && pw1_out_ready;
    assign dw2_mid_out_fire = dw2_mid_out_valid && dw2_mid_out_ready;
    assign stem_output_stall = stem_out_valid && !stem_to_buffer_ready;
    assign dw1_output_stall = dw1_mid_out_valid && !dw1_mid_out_ready;
    assign dw2_output_stall = dw2_mid_out_valid && !dw2_mid_out_ready;
    assign pw1_input_wait = pw1_input_tile_req_valid && !pw1_input_tile_req_ready;
    assign pw1_output_stall = pw1_out_valid && !pw1_out_ready;
    assign pw2_input_wait = pw2_input_tile_req_valid && !pw2_input_tile_req_ready;

    assign front_complete_now = busy
        && pw2_gap_done_seen
        && all_stem_issued
        && !stem_busy
        && !stem_start
        ...
        && !dw2_to_pw2_rd_busy
        && all_pw2_issued
        && (pw2_tiles_done == PW2_TILE_COUNT)
        && !pw2_busy
        && !pw2_start;

    initial begin
        if ((STEM_OC % STEM_OC_LANES) != 0) begin
            $error("stem_dw1_pw1_dw2_pw2_pipeline_top: STEM_OC must be divisible by STEM_OC_LANES");
        end
        if ((PW1_OC % PW1_OC_LANES) != 0) begin
            $error("stem_dw1_pw1_dw2_pw2_pipeline_top: PW1_OC must be divisible by PW1_OC_LANES");
        end
        if ((PW1_OC % CH_LANES) != 0) begin
            $error("stem_dw1_pw1_dw2_pw2_pipeline_top: PW1_OC must be divisible by CH_LANES");
        end
        if ((PW2_OC % PW2_OC_LANES) != 0) begin
            $error("stem_dw1_pw1_dw2_pw2_pipeline_top: PW2_OC must be divisible by PW2_OC_LANES");
        end
        if (STEM_OC != 32) begin
            $error("stem_dw1_pw1_dw2_pw2_pipeline_top: PW1 input channel count assumes STEM_OC=32");
        end
        if (PW1_OC != 64) begin
            $error("stem_dw1_pw1_dw2_pw2_pipeline_top: DW2/PW2 input channel count assumes PW1_OC=64");
        end
        if (PW2_OC != 96 || FC_CLASSES != 5) begin
            $error("stem_dw1_pw1_dw2_pw2_pipeline_top: FC ROM layout assumes PW2_OC=96 and FC_CLASSES=5");
        end
        if (PENDING_DEPTH < 2) begin
            $error("stem_dw1_pw1_dw2_pw2_pipeline_top: PENDING_DEPTH must be >= 2");
        end
        if (DW2_PENDING_DEPTH < 2) begin
            $error("stem_dw1_pw1_dw2_pw2_pipeline_top: DW2_PENDING_DEPTH must be >= 2");
        end
    end
    ...
    stem_to_dw1_buffer_with_ram #(
        ...
        .wr_busy(stem_to_dw1_wr_busy),
        .rd_busy(stem_to_dw1_rd_busy),
        .stem_watermark_stall(stem_to_dw1_watermark_stall)
    );
    ...
    dw1_to_pw1_buffer_with_ram #(
        ...
        .rst_n(rst_n),
        .clear(buffer_clear),
        .dw1_out_valid(dw1_mid_out_valid),
        .dw1_out_ready(dw1_mid_out_ready),
        .dw1_out_t(dw1_mid_out_t),
        .dw1_out_ch_base({1'b0, dw1_mid_out_ch_base}),
        .dw1_out_vec(dw1_mid_out_vec),
        ...
        .act_vec(pw1_act_vec),
        .wr_busy(dw1_to_pw1_wr_busy),
        .rd_busy(dw1_to_pw1_rd_busy),
        .tile_cache_valid(dw1_to_pw1_tile_cache_valid),
        .dw1_overwrite_stall(dw1_to_pw1_overwrite_stall)
    );
    ...
    pw_conv1_array_4x8 #(
        ...
        .act_vec_ready(pw1_act_vec_ready),
        .act_vec(pw1_act_vec),
        .w_fold_we(pw1_w_fold_we),
        .w_fold_oc(pw1_w_fold_oc),
        .w_fold_wdata(pw1_w_fold_wdata),
        .bias_fold_we(pw1_bias_fold_we),
        .bias_fold_oc(pw1_bias_fold_oc),
        .bias_fold_wdata(pw1_bias_fold_wdata),
        ...
        .out_last_group(pw1_out_last_group),
    ...
    pw1_to_dw2_buffer_with_ram #(
        ...
        .pw1_out_pool_t_base(pw1_out_pool_t_base[LOW_TIME_W-1:0]),
        ...
        .wr_busy(pw1_to_dw2_wr_busy),
        .rd_busy(pw1_to_dw2_rd_busy),
        .pw1_overwrite_stall(pw1_to_dw2_overwrite_stall)
    );
    ...
    dw_conv2_4ch_5tap #(
        ...
        .win_vec(dw2_win_vec),
        .weight_we(dw2_weight_we),
        .weight_ch(dw2_weight_ch),
        .weight_tap(dw2_weight_tap),
        .weight_wdata(dw2_weight_wdata),
        ...
        .job_out_t_start(dw2_job_out_t_start),
        .job_out_count(dw2_job_out_count)
    );
    ...
    dw2_to_pw2_buffer_with_ram #(
        ...
        .rst_n(rst_n),
        .clear(buffer_clear),
        .dw2_out_valid(dw2_mid_out_valid),
        .dw2_out_ready(dw2_mid_out_ready),
        .dw2_out_t(dw2_mid_out_t),
        .dw2_out_ch_base({1'b0, dw2_mid_out_ch_base}),
        .dw2_out_vec(dw2_mid_out_vec),
        ...
        .act_vec(pw2_act_vec),
        .wr_busy(dw2_to_pw2_wr_busy),
        .rd_busy(dw2_to_pw2_rd_busy),
        .tile_cache_valid(dw2_to_pw2_tile_cache_valid),
        .dw2_overwrite_stall(dw2_to_pw2_overwrite_stall)
    );
    ...
    pw_conv2_array_4x12 #(
        ...
        .act_vec_ready(pw2_act_vec_ready),
        .act_vec(pw2_act_vec),
        .w_fold_we(pw2_w_fold_we),
        .w_fold_oc(pw2_w_fold_oc),
        .w_fold_wdata(pw2_w_fold_wdata),
        .bias_fold_we(pw2_bias_fold_we),
        .bias_fold_oc(pw2_bias_fold_oc),
        .bias_fold_wdata(pw2_bias_fold_wdata),
        .busy(pw2_busy),
        .done(pw2_done),
        .gap_frame_done(pw2_gap_frame_done_wire),
        .gap_count(gap_count),
    ...
    fc_classifier_96_5 #(
        ...
        .gap_count(gap_count),
        .gap_sum_vec(gap_sum_vec),
        .result_ready(fc_result_ready),
        .busy(fc_busy_wire),
        .done(fc_done_wire),
        .result_valid(fc_result_valid_wire),
        .class_idx(fc_class_idx_wire),
        .input_count_error(fc_input_count_error_wire),
        .logits_vec(fc_logits_vec_wire)
    );
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            dw1_jobs_done <= 32'd0;
            dw1_rows_out <= 32'd0;
            pw1_tiles_started <= 32'd0;
            pw1_tiles_done <= 32'd0;
            pw1_output_tiles <= 32'd0;
            dw2_jobs_started <= 32'd0;
            dw2_jobs_done <= 32'd0;
            dw2_rows_out <= 32'd0;
            pw2_tiles_started <= 32'd0;
            pw2_tiles_done <= 32'd0;
            fc_jobs_started <= 32'd0;
            fc_jobs_done <= 32'd0;
            stem_start_blocked_pending_cycles <= 32'd0;
            stem_output_stall_cycles <= 32'd0;
            dw1_output_stall_cycles <= 32'd0;
            dw2_output_stall_cycles <= 32'd0;
            stem_to_dw1_watermark_stall_cycles <= 32'd0;
            dw1_to_pw1_overwrite_stall_cycles <= 32'd0;
            pw1_to_dw2_overwrite_stall_cycles <= 32'd0;
            dw2_to_pw2_overwrite_stall_cycles <= 32'd0;
            pw1_input_wait_cycles <= 32'd0;
            pw1_output_stall_cycles <= 32'd0;
            pw2_input_wait_cycles <= 32'd0;
            max_dw1_pending_count <= {PENDING_CNT_W{1'b0}};
            max_dw2_pending_count <= {DW2_PENDING_CNT_W{1'b0}};
            for (rst_i = 0; rst_i < PENDING_DEPTH; rst_i = rst_i + 1) begin
                dw1_pending_t_base[rst_i] <= {TIME_W{1'b0}};
                dw1_pending_ch_base[rst_i] <= 5'd0;
                dw1_pending_final_chunk[rst_i] <= 1'b0;
            end
            for (rst_i = 0; rst_i < DW2_PENDING_DEPTH; rst_i = rst_i + 1) begin
                dw2_pending_t_base[rst_i] <= {LOW_TIME_W{1'b0}};
                dw2_pending_ch_base[rst_i] <= 6'd0;
                dw2_pending_final_chunk[rst_i] <= 1'b0;
            end
        end else begin
            done <= 1'b0;
            stem_start <= 1'b0;
            ...
            if (start && !busy) begin
                ...
                if (dw1_output_stall) begin
                    dw1_output_stall_cycles <= dw1_output_stall_cycles + 1'b1;
                end
                if (dw2_output_stall) begin
                    dw2_output_stall_cycles <= dw2_output_stall_cycles + 1'b1;
                end
                if (stem_to_dw1_watermark_stall) begin
                    stem_to_dw1_watermark_stall_cycles
                        <= stem_to_dw1_watermark_stall_cycles + 1'b1;
                end
                if (dw1_to_pw1_overwrite_stall) begin
                    dw1_to_pw1_overwrite_stall_cycles
                        <= dw1_to_pw1_overwrite_stall_cycles + 1'b1;
                end
                if (pw1_to_dw2_overwrite_stall) begin
                    pw1_to_dw2_overwrite_stall_cycles
                        <= pw1_to_dw2_overwrite_stall_cycles + 1'b1;
                end
                if (dw2_to_pw2_overwrite_stall) begin
                    dw2_to_pw2_overwrite_stall_cycles
                        <= dw2_to_pw2_overwrite_stall_cycles + 1'b1;
                end
                if (pw1_input_wait) begin
                    pw1_input_wait_cycles <= pw1_input_wait_cycles + 1'b1;
                end
                if (pw1_output_stall) begin
                    pw1_output_stall_cycles <= pw1_output_stall_cycles + 1'b1;
                end
                if (pw2_input_wait) begin
                    pw2_input_wait_cycles <= pw2_input_wait_cycles + 1'b1;
                end
                if (dw1_mid_out_fire) begin
                    dw1_rows_out <= dw1_rows_out + 1'b1;
                end
                if (dw2_mid_out_fire) begin
                    dw2_rows_out <= dw2_rows_out + 1'b1;
                end
                if (dw1_done) begin
                    dw1_jobs_done <= dw1_jobs_done + 1'b1;
                end
                if (pw1_done) begin
                    pw1_tiles_done <= pw1_tiles_done + 1'b1;
                end
                if (pw1_out_fire) begin
                    pw1_output_tiles <= pw1_output_tiles + 1'b1;
                end
                if (dw2_done) begin
                    dw2_jobs_done <= dw2_jobs_done + 1'b1;
                end
                if (pw2_done) begin
                    pw2_tiles_done <= pw2_tiles_done + 1'b1;
                end
                if (pw2_gap_frame_done_wire) begin
                ...
                if (dw1_pending_push) begin
                    dw1_pending_t_base[dw1_pending_wr_ptr] <= launched_t_base;
                    dw1_pending_ch_base[dw1_pending_wr_ptr] <= launched_ch_base;
                    dw1_pending_final_chunk[dw1_pending_wr_ptr] <= launched_final_chunk;
                    dw1_pending_wr_ptr <= next_dw1_pending_ptr(dw1_pending_wr_ptr);
                ...
                if (dw1_pending_pop) begin
                    ...
                    dw1_input_rows <= ROWS_VALUE;
                    dw1_final_chunk <= dw1_pending_final_chunk[dw1_pending_rd_ptr];
                    dw1_pending_rd_ptr <= next_dw1_pending_ptr(dw1_pending_rd_ptr);
                ...
                if (pw1_start_now) begin
                    ...
                    pw1_tiles_started <= pw1_tiles_started + 1'b1;
                ...
                if (dw2_pending_push) begin
                    ...
                    dw2_pending_ch_base[dw2_pending_wr_ptr] <= pw1_out_oc_base;
                ...
                if (dw2_pending_pop) begin
                    ...
                    dw2_jobs_started <= dw2_jobs_started + 1'b1;
                end
                ...
                if (pw2_start_now) begin
                    ...
                    pw2_tiles_started <= pw2_tiles_started + 1'b1;
                ...
                if (dw1_pending_count_next > max_dw1_pending_count) begin
                    max_dw1_pending_count <= dw1_pending_count_next;
                end
                dw2_pending_count <= dw2_pending_count_next;
                if (dw2_pending_count_next > max_dw2_pending_count) begin
                    max_dw2_pending_count <= dw2_pending_count_next;
                end

                if (front_complete_now && !fc_started && !fc_busy_wire) begin
                    fc_start <= 1'b1;
                    fc_started <= 1'b1;
                    fc_jobs_started <= fc_jobs_started + 1'b1;
                end

                if (fc_done_wire) begin
                    busy <= 1'b0;
                    done <= 1'b1;
                    fc_started <= 1'b0;
                    fc_jobs_done <= fc_jobs_done + 1'b1;
                end
            end
        end
    end
