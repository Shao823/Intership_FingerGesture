...
// Weight address and response controller for LightCNN1D PW2.
//
// The PW2 weight ROM is organized as 512 x 192-bit words:
//   addr = (oc_base / 12) * 64 + k
//   dout = {W[oc_base+11][k], ..., W[oc_base+0][k]}
//
// The generated blk_mem_gen_pw_conv2_weight IP is a 192-bit wide, 512-deep
// single-port ROM with READ_LATENCY_A=1. BRAM_READ_LATENCY=1 matches that IP
// configuration. A small response FIFO keeps one-read-per-cycle throughput
// while absorbing in-flight ROM responses if the compute core stalls
// weight_vec_ready.

module pw_conv2_weight_controller #(
    ...
    initial begin
        if ((PW2_OC % OC_LANES) != 0) begin
            $error("pw_conv2_weight_controller: PW2_OC must be divisible by OC_LANES");
        end
        if (BRAM_READ_LATENCY < 1) begin
            $error("pw_conv2_weight_controller: BRAM_READ_LATENCY must be >= 1");
        end
    end
    ...
    function [ADDR_W-1:0] calc_weight_addr;
        ...
        input [6:0] k_i;
        begin
            calc_weight_addr = ((oc_base_i / OC_LANES) * PW2_IC) + k_i;
        end
    endfunction
...
`ifndef SYNTHESIS
    ...
    assign invalid_oc_range = (weight_req_oc_base > (PW2_OC - OC_LANES));
    assign invalid_k = (weight_req_k >= PW2_IC);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (invalid_oc_align) begin
                $display(
                    "ERROR pw_conv2_weight_controller: oc_base %0d is not %0d-aligned",
                    weight_req_oc_base,
                    OC_LANES
                );
            end
            if (invalid_oc_range) begin
                $display(
                    "ERROR pw_conv2_weight_controller: oc_base %0d exceeds max %0d",
                    weight_req_oc_base,
                    PW2_OC - OC_LANES
                );
            end
            if (invalid_k) begin
                $display(
                    "ERROR pw_conv2_weight_controller: k %0d exceeds max %0d",
                    weight_req_k,
                    PW2_IC - 1
                );
            end
